...
def _child(solver: str) -> dict[str, Any]:
    ...
    pipeline_build_ms = (time.perf_counter() - pipeline_started) * 1000.0

    if solver == "path":
        from witwin.channel_native.path import Config, solve_v2

        operation = lambda: solve_v2(
            scene, Config(max_depth=1, components={"los", "reflection"})
        )
    elif solver == "deterministic":
        from witwin.channel_native.deterministic import Config, solve

        operation = lambda: solve(
            scene, Config(max_depth=1, components={"los", "reflection"})
        )
    elif solver == "basic":
        ...
        operation = lambda: solve(
            scene,
            Config(samples=256, max_depth=1, components={"los", "reflection"}),
        )
    elif solver == "bdpt":
