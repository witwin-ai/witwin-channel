...
__global__ void capacity_finalize_overflow_kernel(
    ...
    for (int64_t pair = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
        ...
        if (static_cast<int64_t>(pair_counts[pair]) > path_capacity_per_pair) {
            ...
            atomicOr(
                failure_state,
                channel::capacity::kPairCapacityOverflow);
        }
    }
}
...
int64_t channel::capacity::deterministic_capacity_validate(
...
channel::capacity::FinalizeState
channel::capacity::deterministic_capacity_finalize_no_trap(
...
void channel::capacity::deterministic_capacity_publish_status(
