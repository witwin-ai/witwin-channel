...
__device__ float channel_neg_kd_phase(float k, float d) {
...
__global__ void deterministic_los_topology_all_kernel(
    ...
    for (int64_t index = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
        ...
        const float phase = channel_neg_kd_phase(2.0f * kPi / wavelength, length);
...
pybind11::dict channel_deterministic_topology_default_fields(at::Tensor reference) {
...
at::Tensor channel_deterministic_repeat_range(at::Tensor reference, int64_t start, int64_t end, int64_t repeats) {
    check_cuda_tensor(reference, "reference", at::kFloat, 1);
...
at::Tensor channel_deterministic_face_anchor_points(at::Tensor vertices, at::Tensor faces) {
...
at::Tensor channel_deterministic_face_sequence_chunk(
    at::Tensor reference,
    int64_t face_count,
    int64_t depth,
    int64_t start,
...
at::Tensor channel_deterministic_mapped_face_sequence_chunk(
    at::Tensor face_ids,
    int64_t depth,
    int64_t start,
    int64_t end,
...
at::Tensor channel_deterministic_normalize_vec3(at::Tensor values, double eps) {
...
at::Tensor channel_deterministic_reflect_points(
    at::Tensor points,
...
std::tuple<
    ...
    at::Tensor,
    at::Tensor>
channel_path_reflection_candidates_cuda(
    at::Tensor vertices,
    at::Tensor faces,
