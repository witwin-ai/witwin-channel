...
__global__ void enumerated_capacity_failure_sanitize_kernel(
    ...
    channel_native::evaluated_paths::PayloadInputView input,
    int64_t *selected_row_index,
    bool *output_valid,
    channel_native::evaluated_paths::PayloadOutputView output,
    int64_t rows,
    ...
    for (int64_t row = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
        ...
        channel_native::evaluated_paths::initialize_row(output, row, sequence_width);
...
pybind11::dict cn_enumerated_capacity_failure_sanitize(
    ...
    channel_native::check_tensor(valid, "valid", at::kBool, 1);
    ...
    const int64_t sequence_width =
        channel_native::evaluated_paths::validate_payload(payload, rows, device);
    ...
    auto output = channel_native::evaluated_paths::allocate_payload(
        valid, rows, sequence_width);
    if (rows > 0) {
        const auto input = channel_native::evaluated_paths::input_view(payload);
        const auto output_view = channel_native::evaluated_paths::output_view(output);
...
at::Tensor cn_enumerated_capacity_failure_vector_sanitize(
    at::Tensor failure_state,
    at::Tensor values) {
    ...
    channel_native::capacity::validate_failure_state(failure_state, values);
