...
ContinuousViews make_views(
    ...
    int device) {
    return {
        optional_view<float>(path_length_m, "path_length_m", at::kFloat, {rows}, device),
        optional_view<float>(delay_s, "delay_s", at::kFloat, {rows}, device),
        optional_view<float>(
            field_direction, "field_direction", at::kFloat, {rows, 3}, device),
        optional_view<float>(
            interaction_position, "interaction_position", at::kFloat, {rows, 3}, device),
        optional_view<float>(
            interaction_normal, "interaction_normal", at::kFloat, {rows, 3}, device),
        optional_view<float>(interaction_positions,
                             "interaction_positions",
                             at::kFloat,
                             {rows, sequence_width, 3},
                             device),
        optional_view<float>(interaction_normals,
                             "interaction_normals",
                             at::kFloat,
                             {rows, sequence_width, 3},
                             device),
        optional_view<float>(path_gain, "path_gain", at::kFloat, {rows}, device),
        optional_view<cfloat>(
            path_field, "path_field", at::kComplexFloat, {rows}, device),
        optional_view<cfloat>(
            field_xyz, "field_xyz", at::kComplexFloat, {rows, 3}, device),
        optional_view<cfloat>(
            coefficient, "coefficient", at::kComplexFloat, {rows}, device)};
}
...
AllocatedContinuous allocate_continuous(
