...
#include <monitors/common/suffix_grid/suffix_grid.h>
#include <monitors/common/suffix_grid/suffix_grid_common.h>
...
__global__ void suffix_grid_jvp_kernel(
    int plane_axis,
    ...
    const float* __restrict__ t_seg_origin_x,
    const float* __restrict__ t_seg_origin_y,
    const float* __restrict__ t_seg_origin_z,
    const float* __restrict__ t_seg_field_re,
    const float* __restrict__ t_seg_field_im,
    const float* __restrict__ t_seg_vec_x_re,
    const float* __restrict__ t_seg_vec_x_im,
    const float* __restrict__ t_seg_vec_y_re,
    const float* __restrict__ t_seg_vec_y_im,
    const float* __restrict__ t_seg_vec_z_re,
    const float* __restrict__ t_seg_vec_z_im,
    float wavelength,
    float k,
    int n_rays,
    ...
    Vec3f origin_t = make_vec3(
        t_seg_origin_x[segment_idx],
        t_seg_origin_y[segment_idx],
        t_seg_origin_z[segment_idx]
    );
    ...
    Complexf field_t = make_complex(t_seg_field_re[segment_idx], t_seg_field_im[segment_idx]);
    Complexf vec_x = make_complex(seg_vec_x_re[segment_idx], seg_vec_x_im[segment_idx]);
    Complexf vec_x_t = make_complex(t_seg_vec_x_re[segment_idx], t_seg_vec_x_im[segment_idx]);
    Complexf vec_y = make_complex(seg_vec_y_re[segment_idx], seg_vec_y_im[segment_idx]);
    Complexf vec_y_t = make_complex(t_seg_vec_y_re[segment_idx], t_seg_vec_y_im[segment_idx]);
    Complexf vec_z = make_complex(seg_vec_z_re[segment_idx], seg_vec_z_im[segment_idx]);
    Complexf vec_z_t = make_complex(t_seg_vec_z_re[segment_idx], t_seg_vec_z_im[segment_idx]);
    bool initial_in_bounds =
    ...
    float d_tangent = -dot(unit, origin_t);
    Complexf factor = distance_factor(wavelength, k, d);
    Complexf factor_t = distance_factor_tangent(wavelength, k, d, d_tangent);
    Complexf field_contrib = cadd(cmul(field_t, factor), cmul(field, factor_t));
    Complexf vec_x_contrib = cadd(cmul(vec_x_t, factor), cmul(vec_x, factor_t));
    Complexf vec_y_contrib = cadd(cmul(vec_y_t, factor), cmul(vec_y, factor_t));
    Complexf vec_z_contrib = cadd(cmul(vec_z_t, factor), cmul(vec_z, factor_t));

    for (int step = 0; step < max_steps; ++step) {
        bool in_bounds = initial_in_bounds && t < ray_blocker;
        if (!in_bounds) {
            break;
        }
...
__global__ void suffix_grid_jvp_resume_kernel(
    int plane_axis,
    float plane_position,
    float coord_0_min,
    float coord_0_max,
    ...
    Vec3f origin_t = make_vec3(t_seg_origin_x[tid], t_seg_origin_y[tid], t_seg_origin_z[tid]);
    ...
    Complexf field_t = make_complex(t_seg_field_re[tid], t_seg_field_im[tid]);
    Complexf vec_x = make_complex(seg_vec_x_re[tid], seg_vec_x_im[tid]);
    Complexf vec_x_t = make_complex(t_seg_vec_x_re[tid], t_seg_vec_x_im[tid]);
    Complexf vec_y = make_complex(seg_vec_y_re[tid], seg_vec_y_im[tid]);
    Complexf vec_y_t = make_complex(t_seg_vec_y_re[tid], t_seg_vec_y_im[tid]);
    Complexf vec_z = make_complex(seg_vec_z_re[tid], seg_vec_z_im[tid]);
    Complexf vec_z_t = make_complex(t_seg_vec_z_re[tid], t_seg_vec_z_im[tid]);
...
__global__ void suffix_grid_jvp_resume_batched_kernel(
    int plane_axis,
    float plane_position,
    float coord_0_min,
    float coord_0_max,
...
void suffix_grid_jvp(
    ...
    suffix_grid_jvp_kernel<<<grid, BLOCK>>>(
        plane_axis,
        plane_position,
        coord_0_min,
        coord_0_max,
    ...
    throw_cuda(cudaGetLastError(), "suffix_grid_jvp_kernel launch");
}
...
void suffix_grid_jvp_resume(
    ...
    constexpr int BLOCK = 256;
    int grid = (n_rays + BLOCK - 1) / BLOCK;
    suffix_grid_jvp_resume_kernel<<<grid, BLOCK>>>(
        plane_axis,
        plane_position,
        coord_0_min,
        coord_0_max,
    ...
    throw_cuda(cudaGetLastError(), "suffix_grid_jvp_resume_kernel launch");
}
...
void suffix_grid_jvp_resume_batched(
    ...
    constexpr int BLOCK = 256;
    int grid = (n_rays + BLOCK - 1) / BLOCK;
    suffix_grid_jvp_resume_batched_kernel<<<grid, BLOCK>>>(
        plane_axis,
        plane_position,
        coord_0_min,
        coord_0_max,
    ...
    throw_cuda(cudaGetLastError(), "suffix_grid_jvp_resume_batched_kernel launch");
}
