...
__device__ __forceinline__ MatchedIsbPrimal compute_matched_isb_primal(
    ...
    primal.tx_norm_sq = dot(tx_basis, tx_basis);
    primal.continued_direct_power = complex_abs_sqr(continued_direct) * primal.tx_norm_sq;
    float response_abs_sqr = complex_abs_sqr(incident_response);
    primal.transition_magnitude = sqrtf(fmaxf(response_abs_sqr, 0.0f));
    primal.transition_phase = atan2f(incident_response.im, incident_response.re);
    return primal;
}
...
struct IncidentStatsPairPrimal {
    ...
    float transition_magnitude;
    float base_weight;
    float target_support_weight;
    float target_support_dweight_dphi;
    Complex response;
    float weight;
...
__device__ __forceinline__ float shadow_boundary_smooth01(float u) {
    float x = fminf(fmaxf(u, 0.0f), 1.0f);
    return x * x * (3.0f - 2.0f * x);
}

__device__ __forceinline__ float shadow_boundary_target_support_weight(
    float phi,
    float wedge_n,
    float& dweight_dphi
) {
    float opening_angle = fmaxf(2.0f * UTD_PI - wedge_n * UTD_PI, 2.0e-3f);
    float angle_ratio = fminf(opening_angle / UTD_PI, 1.0f);
    float decay_span = fmaxf((0.17f + 0.12f * angle_ratio) * opening_angle, 8.0e-3f);
    decay_span = fminf(decay_span, 0.5f * opening_angle);
    float decay_power = 2.0f + (1.0f - angle_ratio);
    float left_distance = phi - wedge_n * UTD_PI;
    float right_distance = 2.0f * UTD_PI - phi;
    bool use_left_boundary = left_distance <= right_distance;
    float distance = use_left_boundary ? left_distance : right_distance;
    float raw_u = distance / fmaxf(decay_span, UTD_EPS);
    float u = fminf(fmaxf(raw_u, 0.0f), 1.0f);
    float smooth = shadow_boundary_smooth01(u);
    float curve = 0.88f * (1.0f - smooth) + 0.12f * (1.0f - u);
    float weight = powf(fmaxf(curve, 0.0f), decay_power);

    float dweight_ddistance = 0.0f;
    if (raw_u > 0.0f && raw_u < 1.0f && curve > UTD_SMALL_EPS) {
        float dsmooth_du = 6.0f * u * (1.0f - u);
        float dcurve_du = -0.88f * dsmooth_du - 0.12f;
        float dweight_du = decay_power * powf(curve, decay_power - 1.0f) * dcurve_du;
        dweight_ddistance = dweight_du / fmaxf(decay_span, UTD_EPS);
    }
    dweight_dphi = dweight_ddistance * (use_left_boundary ? 1.0f : -1.0f);
    return weight;
}
...
__device__ __forceinline__ IncidentStatsPairPrimal compute_shadow_boundary_incident_pair_primal(
    ...
    compute_edge_angles(tx_pos, edge_pos, edge_hat, n0, rx_pos, primal.phi, primal.phi_prime, s_proj, s_prime_proj);
    primal.s_prime = safe_length(f3_sub(edge_pos, tx_pos)) + UTD_EPS;
    primal.s = safe_length(f3_sub(rx_pos, edge_pos)) + UTD_EPS;
    bool source_exterior = wedge_exterior_mask(f3_sub(tx_pos, edge_pos), edge_hat, n0, nn);
    primal.active =
        source_visible
        && (wedge_n > 1.01f)
        && source_exterior
    ...
    primal.target_support_weight = 0.0f;
    primal.target_support_dweight_dphi = 0.0f;
    if (!primal.active) {
    ...
    primal.target_support_weight = shadow_boundary_target_support_weight(
        primal.phi,
        wedge_n,
        primal.target_support_dweight_dphi
    );
    ...
    primal.weight = primal.base_weight * primal.finite_factor.factor_scale * primal.target_support_weight;
    return primal;
}
...
__device__ __forceinline__ void shadow_boundary_incident_pair_jvp(
    ...
    float t_support_weight = primal.target_support_dweight_dphi * t_phi;
    t_weight =
        (
            t_base_weight * primal.finite_factor.factor_scale
            + primal.base_weight * t_factor_scale
        ) * primal.target_support_weight
        + primal.base_weight * primal.finite_factor.factor_scale * t_support_weight;
}
...
__device__ __forceinline__ void shadow_boundary_incident_pair_vjp(
    ...
    float grad_pre_support_weight = grad_weight * primal.target_support_weight;
    float grad_base_weight = grad_pre_support_weight * primal.finite_factor.factor_scale;
    float grad_factor_scale = grad_pre_support_weight * primal.base_weight;
    float grad_support_weight = grad_weight * primal.base_weight * primal.finite_factor.factor_scale;
    ...
    float grad_phi = grad_dif_phi + grad_support_weight * primal.target_support_dweight_dphi;
...
__global__ void radiomap_matched_isb_completion_forward_kernel(
    ...
    float* __restrict__ vector_z_re,
    float* __restrict__ vector_z_im,
    float* __restrict__ continued_direct_power,
    float* __restrict__ transition_magnitude,
    float* __restrict__ transition_phase,
    int n_rx
) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n_rx) {
    ...
    continued_direct_power[tid] = primal.continued_direct_power;
    transition_magnitude[tid] = primal.transition_magnitude;
    transition_phase[tid] = primal.transition_phase;
}
...
__global__ void radiomap_matched_isb_completion_jvp_kernel(
    ...
    float* __restrict__ t_continued_direct_power,
    float* __restrict__ t_transition_magnitude,
    float* __restrict__ t_transition_phase,
    int n_rx
) {
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid >= n_rx) {
    ...
    float response_abs_sqr = complex_abs_sqr(incident_response);
    float safe_response_abs = fmaxf(sqrtf(fmaxf(response_abs_sqr, 0.0f)), RM_TRANSITION_EPS);
    float safe_response_abs_sqr = fmaxf(response_abs_sqr, RM_TRANSITION_EPS);
    float t_transition_mag_value = (
        incident_response.re * t_incident_response.re
        + incident_response.im * t_incident_response.im
    ) / safe_response_abs;
    float t_transition_phase_value = (
        incident_response.re * t_incident_response.im
        - incident_response.im * t_incident_response.re
    ) / safe_response_abs_sqr;
    float t_continued_power_value = 2.0f * (
        primal.tx_norm_sq * complex_real_inner(continued_direct, t_continued_direct)
        + complex_abs_sqr(continued_direct) * dot(tx_basis, t_tx_basis)
    );
    ...
    if (local_interior_mask) {
        ...
        t_continued_direct_power[tid] = t_continued_power_value;
        t_transition_magnitude[tid] = t_transition_mag_value;
        t_transition_phase[tid] = t_transition_phase_value;
        return;
    }
...
__global__ void radiomap_matched_isb_completion_backward_kernel(
    ...
    const float* __restrict__ grad_continued_direct_power,
    const float* __restrict__ grad_transition_magnitude,
    const float* __restrict__ grad_transition_phase,
    ...
    float g_continued_power = grad_continued_direct_power[tid];
    if (g_continued_power != 0.0f) {
        grad_cd.re += 2.0f * g_continued_power * continued_direct.re * primal.tx_norm_sq;
        grad_cd.im += 2.0f * g_continued_power * continued_direct.im * primal.tx_norm_sq;
        float cd_power = complex_abs_sqr(continued_direct);
        grad_tx_basis.x += 2.0f * g_continued_power * cd_power * tx_basis.x;
        grad_tx_basis.y += 2.0f * g_continued_power * cd_power * tx_basis.y;
        grad_tx_basis.z += 2.0f * g_continued_power * cd_power * tx_basis.z;
    }

    float response_abs_sqr = complex_abs_sqr(incident_response);
    float safe_response_abs = fmaxf(sqrtf(fmaxf(response_abs_sqr, 0.0f)), RM_TRANSITION_EPS);
    float safe_response_abs_sqr = fmaxf(response_abs_sqr, RM_TRANSITION_EPS);
    float g_transition_mag = grad_transition_magnitude[tid];
    float g_transition_phase = grad_transition_phase[tid];
    if (g_transition_mag != 0.0f) {
        grad_incident_response.re += g_transition_mag * incident_response.re / safe_response_abs;
        grad_incident_response.im += g_transition_mag * incident_response.im / safe_response_abs;
    }
    if (g_transition_phase != 0.0f) {
        grad_incident_response.re += g_transition_phase * (-incident_response.im) / safe_response_abs_sqr;
        grad_incident_response.im += g_transition_phase * incident_response.re / safe_response_abs_sqr;
    }
...
void radiomap_matched_isb_completion_backward(
    ...
    radiomap_matched_isb_completion_backward_kernel<<<grid_size, block_size>>>(
        ...
        grad_vector_z_re,
        grad_vector_z_im,
        grad_continued_direct_power,
        grad_transition_magnitude,
        grad_transition_phase,
