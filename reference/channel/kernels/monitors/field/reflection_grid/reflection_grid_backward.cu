...
__device__ __forceinline__ void accumulate_reflection_backward_output(
    int cell_idx,
    Vec3f cell_pos,
    Vec3f prev_refl_p,
    Vec3f prev_refl_n,
    ...
    Vec3f& grad_prev_tx,
    Complexf& grad_prev_weight,
    Complexf& grad_prev_pol_x,
    Complexf& grad_prev_pol_y,
    Complexf& grad_prev_pol_z
) {
    ...
    reflection_grid_detail::accumulate_complex_backward(
        prev_weight,
        factor,
        make_complex(grad_field_re[cell_idx], grad_field_im[cell_idx]),
        grad_prev_weight,
        grad_factor
    );
    reflection_grid_detail::accumulate_complex_backward(
        prev_pol_x,
        factor,
        make_complex(grad_pol_x_re[cell_idx], grad_pol_x_im[cell_idx]),
        grad_prev_pol_x,
        grad_factor
    );
    reflection_grid_detail::accumulate_complex_backward(
        prev_pol_y,
        factor,
        make_complex(grad_pol_y_re[cell_idx], grad_pol_y_im[cell_idx]),
        grad_prev_pol_y,
        grad_factor
    );
    reflection_grid_detail::accumulate_complex_backward(
        prev_pol_z,
        factor,
        make_complex(grad_pol_z_re[cell_idx], grad_pol_z_im[cell_idx]),
        grad_prev_pol_z,
        grad_factor
    );

    float grad_d = factor_distance_adjoint(wavelength, k, d, grad_factor);
    Vec3f grad_mirror = mul(unit, -grad_d);
    mirror_backward(
        grad_mirror,
        prev_tx,
        prev_refl_p,
...
__global__ void reflection_grid_backward_kernel(
    int plane_axis,
    ...
    const float* __restrict__ grad_pol_x_re,
    const float* __restrict__ grad_pol_x_im,
    const float* __restrict__ grad_pol_y_re,
    const float* __restrict__ grad_pol_y_im,
    const float* __restrict__ grad_pol_z_re,
    const float* __restrict__ grad_pol_z_im,
    float* __restrict__ grad_prev_refl_p_x,
    float* __restrict__ grad_prev_refl_p_y,
    float* __restrict__ grad_prev_refl_p_z,
    float* __restrict__ grad_prev_refl_n_x,
    float* __restrict__ grad_prev_refl_n_y,
    float* __restrict__ grad_prev_refl_n_z,
    float* __restrict__ grad_prev_tx_x,
    float* __restrict__ grad_prev_tx_y,
    float* __restrict__ grad_prev_tx_z,
    float* __restrict__ grad_prev_weight_re,
    float* __restrict__ grad_prev_weight_im,
    float* __restrict__ grad_prev_pol_x_re,
    float* __restrict__ grad_prev_pol_x_im,
    float* __restrict__ grad_prev_pol_y_re,
    float* __restrict__ grad_prev_pol_y_im,
    float* __restrict__ grad_prev_pol_z_re,
    float* __restrict__ grad_prev_pol_z_im
) {
    ...
    Vec3f grad_prev_refl_p = make_vec3(0.0f, 0.0f, 0.0f);
    Vec3f grad_prev_refl_n = make_vec3(0.0f, 0.0f, 0.0f);
    Vec3f grad_prev_tx = make_vec3(0.0f, 0.0f, 0.0f);
    Complexf grad_prev_weight = make_complex(0.0f, 0.0f);
    Complexf grad_prev_pol_x = make_complex(0.0f, 0.0f);
    Complexf grad_prev_pol_y = make_complex(0.0f, 0.0f);
    Complexf grad_prev_pol_z = make_complex(0.0f, 0.0f);
    ...
    if (plane_axis == 2) {
        ...
        for (int step = 0; step < max_steps; ++step) {
            ...
            accumulate_reflection_backward_output(
                cell_idx,
                cell_pos,
                prev_refl_p,
                prev_refl_n,
    ...
    grad_prev_refl_p_x[tid] = grad_prev_refl_p.x;
    grad_prev_refl_p_y[tid] = grad_prev_refl_p.y;
    grad_prev_refl_p_z[tid] = grad_prev_refl_p.z;
    grad_prev_refl_n_x[tid] = grad_prev_refl_n.x;
    grad_prev_refl_n_y[tid] = grad_prev_refl_n.y;
    grad_prev_refl_n_z[tid] = grad_prev_refl_n.z;
    grad_prev_tx_x[tid] = grad_prev_tx.x;
    grad_prev_tx_y[tid] = grad_prev_tx.y;
    grad_prev_tx_z[tid] = grad_prev_tx.z;
    grad_prev_weight_re[tid] = grad_prev_weight.re;
    grad_prev_weight_im[tid] = grad_prev_weight.im;
    grad_prev_pol_x_re[tid] = grad_prev_pol_x.re;
    grad_prev_pol_x_im[tid] = grad_prev_pol_x.im;
    grad_prev_pol_y_re[tid] = grad_prev_pol_y.re;
    grad_prev_pol_y_im[tid] = grad_prev_pol_y.im;
    grad_prev_pol_z_re[tid] = grad_prev_pol_z.re;
    grad_prev_pol_z_im[tid] = grad_prev_pol_z.im;
}
...
void reflection_grid_backward(
    ...
    reflection_grid_backward_kernel<<<grid, BLOCK>>>(
        plane_axis,
        plane_position,
        coord_0_min,
        coord_0_max,
    ...
    throw_cuda(cudaGetLastError(), "reflection_grid_backward_kernel launch");
}
