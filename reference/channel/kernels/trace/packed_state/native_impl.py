...
from witwin.channel.kernels.trace.packed_state.drjit_impl import (
...
from witwin.channel.trace.diffraction.state.arrays import _empty_state_arrays
...
from witwin.channel.utils.polarization import vector_from_jones
...
def _strip_packed_metadata(state_arrays: dict | None) -> dict | None:
...
def _empty_inserted_reflection_state_fields(
    ...
    *,
    ...
    return _attach_state_metadata(selected, None, history_size)
...
def _inserted_reflection_state_fields_from_tuple(
    ...
    *,
    ...
    if source_state_arrays is not None and indices is not None:
        ...
        metadata = _gather_state_metadata(source_state_arrays, indices)
    return _attach_state_metadata(selected, metadata, history_size)
...
def _pack_core_arrays(state_arrays: dict) -> list:
    return [
        ...
        state_arrays[SK_FACE0_ETA_R], state_arrays[SK_FACE0_SIGMA], state_arrays[SK_FACE0_GAIN],
        state_arrays[SK_FACE0_USE_FRESNEL],
        state_arrays[SK_FACE1_ETA_R], state_arrays[SK_FACE1_SIGMA], state_arrays[SK_FACE1_GAIN],
...
def _state_arrays_from_unpacked_lists(
    ...
    *,
    ...
    _attach_state_metadata(state_arrays, None, history_size)
    if cache_packed:
...
def _unpack_state_arrays(
    ...
    ext,
    *,
    retain_cold_metadata: bool = False,
    cache_packed: bool = False,
) -> dict:
    stride = _packed_stride(ext, history_size)
...
def _pack_state_arrays(state_arrays: dict, ext, history_size: int | None = None):
    history_size_in = _state_history_size(state_arrays)
...
class _PackedStateGatherOp(dr.CustomOp):
    def eval(self, state_arrays: dict, indices):
        ...
        self._input_state_arrays = _strip_packed_metadata(state_arrays)
        self._indices = indices
        history_size = _state_history_size(state_arrays)
        can_use_native, ext = _can_use_native_primal(state_arrays, history_size)
        if not can_use_native:
            self._ext = None
            self._output_state_arrays = _strip_packed_metadata(_drjit_gather(state_arrays, indices))
            return self._output_state_arrays
        self._ext = ext
        self._output_state_arrays = _strip_packed_metadata(
...
def gather_state_arrays(state_arrays: dict, indices) -> dict:
    history_size = 0 if state_arrays is None else _state_history_size(state_arrays)
...
def concat_state_arrays(state_arrays_list: list[dict]) -> dict:
    ...
    if len(non_empty) == 0:
        ...
        return _empty_state_arrays(
            history_size=history_size,
            retain_cold_metadata=retain_cold,
        )
    if len(non_empty) == 1:
        return non_empty[0]
