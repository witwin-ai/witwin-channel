...
Interface matches native/pruning_sort/pruning_sort.h.
...
from witwin.channel.utils import drjit_to_torch_view, torch_lexsort
from witwin.channel.utils.drjit_ops import complex_abs_sqr  # standalone helper
...
def _prepare_pruning_sort_inputs(state_arrays):
    ...
    metric = drjit_to_torch_view(_state_pruning_metric(state_arrays), detach=True)
    ...
    edge_history = [
        drjit_to_torch_view(path_edge_history[slot], detach=True).to(dtype=torch.int32)
        for slot in range(min(history_size, len(path_edge_history)))
    ]
    reflection_history = [
        drjit_to_torch_view(path_reflection_history[slot], detach=True).to(dtype=torch.int32)
    ...
    return torch_lexsort(sort_keys), history_size, n_states
