...
UTD_DINLINE float safe_acos(float v) {
    ...
    float s = sqrtf(fmaxf(1.f - c*c, 0.f) + UTD_SMALL_EPS);
    return atan2f(s, c);
}
...
UTD_DINLINE float cot_val(float v) {
    ...
    c = cosf(v);
#endif
    float d = (fabsf(s) < UTD_SMALL_EPS) ? (s >= 0.f ? UTD_SMALL_EPS : -UTD_SMALL_EPS) : s;
    float r = c / d;
...
// ===================================================================
// Beta term metadata + assembly
// ===================================================================
UTD_DINLINE void beta_term_metadata(float beta, float n, float kL, float cotSign,
    ...
    Complex& sG, Complex& sG1, Complex& dG2, Complex& sG2)
{
    ...
    float kL = k*l;
    float dP = phi-phiP, sP2 = phi+phiP;
    factor = cplx_mul_real(cplx_exp_phase(-0.25f*UTD_PI),
                           -1.f/(2.f*n*sqrtf(UTD_TWO_PI*k+UTD_EPS)));
    float cv[4],c1[4],c2[4],xv[4],x1[4],x2[4];
    beta_term_metadata(dP, n, kL, +1.f, true,  cv[0],c1[0],c2[0],xv[0],x1[0],x2[0]);
    beta_term_metadata(dP, n, kL, -1.f, false, cv[1],c1[1],c2[1],xv[1],x1[1],x2[1]);
    beta_term_metadata(sP2,n, kL, +1.f, true,  cv[2],c1[2],c2[2],xv[2],x1[2],x2[2]);
    beta_term_metadata(sP2,n, kL, -1.f, false, cv[3],c1[3],c2[3],xv[3],x1[3],x2[3]);
    Complex tr[4],tr1[4],tr2[4];
    for (int i=0;i<4;++i) f_utd_with_derivatives(xv[i],tr[i],tr1[i],tr2[i]);
    Complex tv[4],tf[4],ts[4];
    for (int i=0;i<4;++i) assemble_beta_term(cv[i],c1[i],c2[i],x1[i],x2[i],tr[i],tr1[i],tr2[i],tv[i],tf[i],ts[i]);
...
// Fresnel reflection
// ===================================================================
UTD_DINLINE void fresnel_reflection_face(float cosTheta, float etaR, float sigma,
    float omega, Complex& rTE, Complex& rTM)
{
    ...
    Complex a = cplx_sqrt(cplx_sub(eta, cplx(sinSq, 0)));
    rTE = cplx_div(cplx_sub(cplx(ct,0), a), cplx_add(cplx(ct,0), a));
    ...
    float& gSigma)
{
    ...
    float so = fmaxf(omega, UTD_SMALL_EPS);
    Complex eta = cplx(etaR, -sigma / (so * UTD_EPSILON_0));
    Complex tmp = cplx_sub(eta, cplx(sinSq, 0.f));
    Complex a = cplx_sqrt(tmp);

    Complex ctC = cplx(ct, 0.f);
    Complex numTE = cplx_sub(ctC, a);
    Complex denTE = cplx_add(ctC, a);
    ...
    Complex gCtC = cplx_add(gNumTE, gDenTE);
...
{
    float3a faceS = f3_cross(inHat, normal);
    float3a fallbackIn = stable_perp_basis(inHat, make_f3(0, 1, 0));
    Basis3 fIn = basis_from_first_vector(inHat, faceS, fallbackIn);
    float3a fallbackOut = stable_perp_basis(outHat, make_f3(0, 1, 0));
    Basis3 fOut = basis_from_first_vector(outHat, faceS, fallbackOut);
    ...
    float3a gFaceS = f3_zero();
    float3a gFallbackIn = f3_zero();
    adj_basis_from_first_vector(inHat, faceS, fallbackIn, gFIn, gInHat, gFaceS, gFallbackIn);
    float3a gPreferredIn = f3_zero();
    adj_stable_perp_basis(inHat, make_f3(0, 1, 0), gFallbackIn, gInHat, gPreferredIn);

    float3a gFallbackOut = f3_zero();
    adj_basis_from_first_vector(outHat, faceS, fallbackOut, gFOut, gOutHat, gFaceS, gFallbackOut);
    float3a gPreferredOut = f3_zero();
    adj_stable_perp_basis(outHat, make_f3(0, 1, 0), gFallbackOut, gOutHat, gPreferredOut);

    gInHat = f3_add(gInHat, f3_cross(normal, gFaceS));
    gNormal = f3_add(gNormal, f3_cross(gFaceS, inHat));
    (void) gPreferredIn;
    (void) gPreferredOut;
}
    ...
    Basis3 inEdgeBasis, Basis3 outEdgeBasis, float omega)
{
    ...
    Complex rTE, rTM;
    fresnel_reflection_face(cosTheta, fm.etaR, fm.sigma, omega, rTE, rTM);
    ...
    float3a faceS = f3_cross(inHat, normal);
    Basis3 fIn  = basis_from_first_vector(inHat,  faceS, stable_perp_basis(inHat,  make_f3(0,1,0)));
    Basis3 fOut = basis_from_first_vector(outHat, faceS, stable_perp_basis(outHat, make_f3(0,1,0)));
    ...
    float wedgeN, float k, float s, float sP)
{
    float l = s*sP/(s+sP+UTD_EPS);
    float kL = k*l;
    float dPhi = phi-phiP, sPhi = phi+phiP;
    // Build beta term caches inline
    float cv[4],c1v[4],c2v[4],xv[4],x1v[4],x2v[4];
    beta_term_metadata(dPhi, wedgeN, kL, +1.f, true,  cv[0],c1v[0],c2v[0],xv[0],x1v[0],x2v[0]);
    beta_term_metadata(dPhi, wedgeN, kL, -1.f, false, cv[1],c1v[1],c2v[1],xv[1],x1v[1],x2v[1]);
    beta_term_metadata(sPhi, wedgeN, kL, +1.f, true,  cv[2],c1v[2],c2v[2],xv[2],x1v[2],x2v[2]);
    beta_term_metadata(sPhi, wedgeN, kL, -1.f, false, cv[3],c1v[3],c2v[3],xv[3],x1v[3],x2v[3]);
    Complex tr[4],tr1[4],tr2[4];
    for (int i=0;i<4;++i) f_utd_with_derivatives(xv[i],tr[i],tr1[i],tr2[i]);
    Complex tv[4],tf[4],ts[4];
    for (int i=0;i<4;++i) assemble_beta_term(cv[i],c1v[i],c2v[i],x1v[i],x2v[i],tr[i],tr1[i],tr2[i],tv[i],tf[i],ts[i]);
    ...
    float& gPhi, float& gPhiP, float& gWedgeN, float& gS, float& gSP, float& gSinBeta0)
{
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        beta_term_metadata(beta, wedgeN, kL, cotSign, plusBranch, cotV[i], c1Dummy, c2Dummy, x0Dummy, x1Dummy, x2Dummy);
        Complex tr2Dummy;
...
UTD_DINLINE Complex finite_wedge_truncation_factor(PairInputs state, float3a tgtPos, float k) {
    ...
    fresnel_boersma(scale * (state.edgeLineMin - stationaryU), fMin, fMin1, fMin2);
    fresnel_boersma(scale * (state.edgeLineMax - stationaryU), fMax, fMax1, fMax2);
    ...
    Complex& directGain, Complex& derivativeGain)
{
    ...
    geomValid = srcExt && tgtExt && (sP > UTD_MIN_DISTANCE) && (s > UTD_MIN_DISTANCE);
    if (!geomValid) return;
    ...
    Complex d = useFace ? diff_coeff_3d(phi,phiP,w,k,s,sP,sb,r0,rn)
    ...
    Complex finiteFactor = finite_wedge_truncation_factor(state, tgtPos, k);
    directGain = cplx_mul(directGain, finiteFactor);
    derivativeGain = cplx_mul(derivativeGain, finiteFactor);
    field = cplx_add(cplx_mul(state.incidentField, directGain),
                     cplx_mul(state.incidentNormalDerivative, derivativeGain));
}
    ...
    float k, MaterialParams mat)
{
    ...
    float phi,phiP,s,sP,sb;
    compute_edge_geometry_3d(state.sourcePos, state.edgePos, state.edgeDir, state.n0, tgtPos, phi,phiP,s,sP,sb);
    bool geomValid = srcExt && tgtExt && (sP > UTD_MIN_DISTANCE) && (s > UTD_MIN_DISTANCE);
    ...
    Complex3 incidentVector = vector_from_jones(state.incidentJones, state.incidentBasis);
    ...
    JonesOperator slopeOp = assemble_diff_operator(
        cplx_mul(slopeFactor, cplx_mul_real(terms.directDphiPrime, -1.f)),
        cplx_mul(slopeFactor, terms.face0DphiPrime),
        cplx_mul(slopeFactor, terms.face1DphiPrime),
        f0Op, f1Op);

    Jones2 fieldJ = jones_add(apply_jop(incJE, directOp), apply_jop(incDJE, slopeOp));
    Complex finiteFactor = finite_wedge_truncation_factor(state, tgtPos, k);
    ...
    float3a& gTgt)
{
    ...
    bool tgtExt = wedge_exterior_mask(f3_sub(tgt, pi.edgePos), pi.edgeDir, pi.n0, pi.nn);
    float phi, phiP, s, sP, sb;
    ...
    Jones2 incJE  = jones_from_vector(pi.incidentVector, inEB);
    Jones2 incDJE = jones_from_vector(pi.incidentDerivativeVector, inEB);
    ...
    Jones2 slopeFieldJ = apply_jop(incDJE, slopeOp);
    Jones2 fieldJ = jones_add(directFieldJ, slopeFieldJ);
    float ls = sqrtf(sP / (s * (s + sP) + UTD_EPS));
    Complex phase = cplx_exp_phase(-k * s);
    Complex scale = cplx_mul_real(phase, ls);
    Complex3 transport = vector_from_jones(fieldJ, outEB);
    ...
    adj_vector_from_jones(fieldJ, outEB, gTransport, gFieldJ, gOutEB);
    ...
    adj_jones_from_vector(pi.incidentVector, inEB, gIncJE, sg.incidentVector, gInEB);
    adj_jones_from_vector(pi.incidentDerivativeVector, inEB, gIncDJE, sg.incidentDerivativeVector, gInEB);
    ...
    Complex slopeDirectCoeff = cplx_mul(slopeFactor, cplx_mul_real(terms.directDphiPrime, -1.f));
    Complex slopeFace0Coeff = cplx_mul(slopeFactor, terms.face0DphiPrime);
    Complex slopeFace1Coeff = cplx_mul(slopeFactor, terms.face1DphiPrime);
