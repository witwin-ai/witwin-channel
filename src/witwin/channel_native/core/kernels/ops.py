    ...
    *,
    ...
    for name in ("tx_id", "sample_id", "stream_id"):
        ...
        if exported[name].shape != (expected,):
            raise ValueError(f"_channel_native.bdpt_launch_state returned bad {name} shape")
...
def _validate_bdpt_subpath_state(name: str, exported: dict[str, torch.Tensor], expected_count: int | None) -> None:
    if not isinstance(exported, dict):
    ...
    for field, (dtype, shape_spec) in _BDPT_SUBPATH_SCHEMA.items():
        ...
        expected_shape = tuple(inferred_count if dim is None else dim for dim in shape_spec)
...
def _validate_bdpt_connection_samples(
    ...
    for field, (dtype, shape_spec) in _BDPT_CONNECTION_SCHEMA.items():
        tensor = exported[field]
        validate_cuda_tensor(f"{name}.{field}", tensor, dtype=dtype, ndim=len(shape_spec))
        if inferred_count is None:
            inferred_count = int(tensor.shape[0])
        expected_shape = tuple(inferred_count if dim is None else dim for dim in shape_spec)
        if tuple(tensor.shape) != expected_shape:
...
def bdpt_empty_subpath_state(reference: torch.Tensor) -> dict[str, torch.Tensor]:
    ...
    _validate_bdpt_subpath_state("_channel_native.bdpt_empty_subpath_state", exported, 0)
    return exported
...
def bdpt_endpoint_subpath_state(
    ...
    if not isinstance(exported, dict) or set(exported) != {"light", "sensor"}:
        raise TypeError("_channel_native.bdpt_endpoint_subpath_state must return light/sensor dicts")
    light = exported["light"]
    sensor = exported["sensor"]
    _validate_bdpt_subpath_state("_channel_native.bdpt_endpoint_subpath_state.light", light, int(launch_tx_id.shape[0]))
    _validate_bdpt_subpath_state("_channel_native.bdpt_endpoint_subpath_state.sensor", sensor, int(rx_positions.shape[0]))
...
def bdpt_subpath_intersection_inputs(subpath: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
    ...
    if not isinstance(exported, dict):
        raise TypeError("_channel_native.bdpt_subpath_intersection_inputs must return a dict")
    if set(exported) != {"ray_o", "ray_d", "ray_tmax", "active"}:
        raise ValueError("_channel_native.bdpt_subpath_intersection_inputs returned unexpected fields")
    validate_cuda_tensor("ray_o", exported["ray_o"], dtype=torch.float32, ndim=2, trailing_shape=(3,))
    validate_cuda_tensor("ray_d", exported["ray_d"], dtype=torch.float32, ndim=2, trailing_shape=(3,))
    ...
    if exported["ray_o"].shape != subpath["origin"].shape or exported["ray_d"].shape != subpath["direction"].shape:
        raise ValueError("_channel_native.bdpt_subpath_intersection_inputs returned bad ray shape")
    if exported["active"].shape != subpath["valid"].shape:
        raise ValueError("_channel_native.bdpt_subpath_intersection_inputs returned bad active shape")
    if exported["ray_tmax"].shape != (0,):
        raise ValueError("_channel_native.bdpt_subpath_intersection_inputs returned bad ray_tmax shape")
    return exported
...
def bdpt_reflected_light_subpath_state(
    ...
    *,
    ...
    validate_cuda_tensor("intersection.t", intersection["t"], dtype=torch.float32, ndim=1)
    validate_cuda_tensor("intersection.p", intersection["p"], dtype=torch.float32, ndim=2, trailing_shape=(3,))
    validate_cuda_tensor("intersection.n", intersection["n"], dtype=torch.float32, ndim=2, trailing_shape=(3,))
...
def bdpt_endpoint_connection_samples(
    ...
    *,
    ...
    expected_count = expected_total if max_paths is None else min(int(max_paths), expected_total)
    ...
    _validate_bdpt_connection_samples("_channel_native.bdpt_endpoint_connection_samples", exported, expected_count)
    return exported
...
def bdpt_endpoint_connection_visibility_inputs(
    ...
    *,
    ...
    if not isinstance(exported, dict):
        raise TypeError("_channel_native.bdpt_endpoint_connection_visibility_inputs must return a dict")
    if set(exported) != {"start", "end", "active"}:
        raise ValueError("_channel_native.bdpt_endpoint_connection_visibility_inputs returned unexpected fields")
    validate_cuda_tensor("start", exported["start"], dtype=torch.float32, ndim=2, trailing_shape=(3,))
    validate_cuda_tensor("end", exported["end"], dtype=torch.float32, ndim=2, trailing_shape=(3,))
    validate_cuda_tensor("active", exported["active"], dtype=torch.bool, ndim=1)
    if tuple(exported["start"].shape) != (int(sample_count), 3):
        raise ValueError("_channel_native.bdpt_endpoint_connection_visibility_inputs returned bad start shape")
    if exported["end"].shape != exported["start"].shape or exported["active"].shape != (int(sample_count),):
        raise ValueError("_channel_native.bdpt_endpoint_connection_visibility_inputs returned bad visibility shape")
    return exported
...
def bdpt_accumulate_connection_samples(
    ...
    *,
    ...
    if accumulation_strategy not in strategy_ids:
        raise ValueError("accumulation_strategy must be 'atomic', 'staged', or 'compact'")
    ...
    if not isinstance(exported, dict):
        raise TypeError("_channel_native.bdpt_accumulate_connection_samples must return a dict")
    if set(exported) != {"path_gain", "los", "reflection", "diffraction"}:
        raise ValueError("_channel_native.bdpt_accumulate_connection_samples returned unexpected fields")
    for name, tensor in exported.items():
        ...
        if tuple(tensor.shape) != (int(tx_count), int(rx_count)):
            raise ValueError(f"_channel_native.bdpt_accumulate_connection_samples returned bad {name} shape")
    return exported
...
def bdpt_filter_connection_samples(
    ...
    _validate_bdpt_connection_samples("_channel_native.bdpt_filter_connection_samples", exported, None)
    return exported
...
def bdpt_count_valid_connection_samples(samples: dict[str, torch.Tensor]) -> int:
    ...
    if not isinstance(count, int):
        raise TypeError("_channel_native.bdpt_count_valid_connection_samples must return an int")
    if count < 0 or count > int(samples["valid"].shape[0]):
        raise ValueError("_channel_native.bdpt_count_valid_connection_samples returned bad count")
    return count
...
def bdpt_compact_connection_samples(
    ...
    *,
    ...
    if max_paths_value < -1:
        raise ValueError("max_paths must be non-negative")
    exported = _required_native_op("bdpt_compact_connection_samples")(samples, int(max_paths_value))
    _validate_bdpt_connection_samples("_channel_native.bdpt_compact_connection_samples", exported, None)
    if max_paths is not None and int(exported["valid"].shape[0]) > int(max_paths):
        raise ValueError("_channel_native.bdpt_compact_connection_samples exceeded max_paths")
    return exported
...
def bdpt_concat_connection_samples(
    ...
    _validate_bdpt_connection_samples("_channel_native.bdpt_concat_connection_samples", exported, expected_count)
    return exported
...
def deterministic_component_counts(component_id: torch.Tensor) -> dict[str, int]:
    ...
    if not isinstance(exported, dict):
        raise TypeError("_channel_native.deterministic_component_counts must return a dict")
    counts: dict[str, int] = {}
    for name in ("los", "reflection", "diffraction"):
        value = exported[name]
        if not isinstance(value, int):
            raise TypeError(f"_channel_native.deterministic_component_counts returned non-int {name}")
        counts[name] = value
    return counts
...
def deterministic_selected_edge_count(edge_id: torch.Tensor) -> int:
    ...
    if not isinstance(value, int):
        raise TypeError("_channel_native.deterministic_selected_edge_count must return an int")
    return value
...
def core_diffraction_edge_count(
    *,
    ...
    if not isinstance(value, int):
        raise TypeError("_channel_native.core_diffraction_edge_count must return an int")
    return value
...
def bdpt_mis_weights(
    ...
    *,
    ...
    validate_cuda_tensor("strategy_pdf_sum", strategy_pdf_sum, dtype=torch.float32, ndim=0)
    mode_id = _bdpt_mis_mode_id(mis)
    if beta <= 0.0:
        raise ValueError("beta must be positive")
    ...
    if weights.shape != pdf.shape:
        raise ValueError("_channel_native.bdpt_mis_weights returned an unexpected shape")
    return weights
...
def bdpt_diffraction_point_connection_samples(
    ...
    *,
    ...
    if not isinstance(exported, dict):
        raise TypeError("_channel_native.bdpt_diffraction_point_connection_samples must return a dict")
    expected = {"samples", "source_start", "source_end", "target_start", "target_end", "visibility_active"}
    if set(exported) != expected:
        raise ValueError("_channel_native.bdpt_diffraction_point_connection_samples returned unexpected fields")
    sample_count = int(rx_positions.shape[0]) * (int(direct_samples) + int(keller_samples))
    if state_count == 0:
        sample_count = 0
    samples = exported["samples"]
    if not isinstance(samples, dict):
        raise TypeError("_channel_native.bdpt_diffraction_point_connection_samples samples must be a dict")
    _validate_bdpt_connection_samples(
    ...
    for name in ("source_start", "source_end", "target_start", "target_end"):
        ...
        if tuple(tensor.shape) != (sample_count, 3):
            raise ValueError(f"_channel_native.bdpt_diffraction_point_connection_samples returned bad {name} shape")
    ...
    if tuple(active.shape) != (sample_count,):
        raise ValueError("_channel_native.bdpt_diffraction_point_connection_samples returned bad visibility_active shape")
    return exported
...
def bdpt_zero_matrix(reference: torch.Tensor, *, rows: int, cols: int) -> torch.Tensor:
    ...
    if out.shape != (int(rows), int(cols)):
        raise ValueError("_channel_native.bdpt_zero_matrix returned an unexpected shape")
    return out
...
def bdpt_store_point_component_column(
    ...
    *,
    ...
    if not isinstance(out, torch.Tensor):
        raise TypeError("_channel_native.bdpt_store_point_component_column must return a tensor")
    validate_cuda_tensor("out", out, dtype=torch.float32, ndim=2)
    if out.shape != target.shape:
        raise ValueError("_channel_native.bdpt_store_point_component_column returned an unexpected shape")
    return out
...
def bdpt_finalize_point_components(
    ...
    exported = _required_native_op("bdpt_finalize_point_components")(los, reflection, diffraction)
    if not isinstance(exported, dict):
        raise TypeError("_channel_native.bdpt_finalize_point_components must return a dict")
    validate_cuda_tensor("path_gain", exported["path_gain"], dtype=torch.float32, ndim=2)
    if exported["path_gain"].shape != los.shape:
        raise ValueError("_channel_native.bdpt_finalize_point_components returned bad path_gain shape")
...
def bdpt_point_component_power(path_gain: torch.Tensor, *, include_los: bool) -> dict[str, torch.Tensor]:
    ...
    if native is None or not hasattr(native, "bdpt_point_component_power"):
        raise RuntimeError("_channel_native.bdpt_point_component_power CUDA kernel is required")
...
def bdpt_receiver_grid_points(
    ...
    *,
    ...
    if not isinstance(points, torch.Tensor):
        raise TypeError("_channel_native.bdpt_receiver_grid_points must return a tensor")
    validate_cuda_tensor("points", points, dtype=torch.float32, ndim=2, trailing_shape=(3,))
    if points.shape[0] != rows * cols:
        raise ValueError("_channel_native.bdpt_receiver_grid_points returned an unexpected shape")
    return points
...
def bdpt_los_export(
    ...
    *,
    ...
    validate_cuda_tensor("path_gain_matrix", exported["path_gain_matrix"], dtype=torch.float32, ndim=2)
    return exported
...
def bdpt_los_component_maps(los: torch.Tensor) -> torch.Tensor:
    ...
    if maps.shape != los.shape:
        raise ValueError("_channel_native.bdpt_los_component_maps returned an unexpected shape")
    return maps
...
def bdpt_los_component_maps_from_matrix(los: torch.Tensor, *, rows: int, cols: int) -> torch.Tensor:
    ...
    maps = _required_native_op("bdpt_los_component_maps_from_matrix")(los, int(rows), int(cols))
    if not isinstance(maps, torch.Tensor):
        raise TypeError("_channel_native.bdpt_los_component_maps_from_matrix must return a tensor")
    validate_cuda_tensor("maps", maps, dtype=torch.float32, ndim=3)
    if maps.shape != (los.shape[0], int(cols), int(rows)):
        raise ValueError("_channel_native.bdpt_los_component_maps_from_matrix returned an unexpected shape")
    return maps
...
def bdpt_los_visibility_inputs(
    ...
    *,
    ...
    exported = _required_native_op("bdpt_los_visibility_inputs")(tx_positions, int(tx_index), int(rx_count))
    if not isinstance(exported, dict):
...
def bdpt_apply_los_visibility(
    ...
    *,
    ...
    out = _required_native_op("bdpt_apply_los_visibility")(maps, los, visible, int(tx_index))
    if not isinstance(out, torch.Tensor):
        raise TypeError("_channel_native.bdpt_apply_los_visibility must return a tensor")
...
def bdpt_component_map_buffer(
    ...
    *,
    ...
    maps = _required_native_op("bdpt_component_map_buffer")(reference, int(tx_count), int(dim0), int(dim1))
    if not isinstance(maps, torch.Tensor):
        raise TypeError("_channel_native.bdpt_component_map_buffer must return a tensor")
    validate_cuda_tensor("maps", maps, dtype=torch.float32, ndim=3)
    if maps.shape != (tx_count, dim0, dim1):
        raise ValueError("_channel_native.bdpt_component_map_buffer returned an unexpected shape")
    return maps
...
def bdpt_store_scaled_component_map(
    ...
    *,
    ...
    if not isinstance(out, torch.Tensor):
        raise TypeError("_channel_native.bdpt_store_scaled_component_map must return a tensor")
    validate_cuda_tensor("maps", out, dtype=torch.float32, ndim=3)
    return out
...
def bdpt_finalize_component_maps(
    ...
    exported = _required_native_op("bdpt_finalize_component_maps")(los, reflection, diffraction)
    if not isinstance(exported, dict):
        raise TypeError("_channel_native.bdpt_finalize_component_maps must return a dict")
    validate_cuda_tensor("path_gain", exported["path_gain"], dtype=torch.float32, ndim=3)
    if exported["path_gain"].shape != los.shape:
        raise ValueError("_channel_native.bdpt_finalize_component_maps returned bad path_gain shape")
    for name in ("los_power", "reflection_power", "diffraction_power"):
        validate_cuda_tensor(name, exported[name], dtype=torch.float32, ndim=0)
    return exported
...
def bdpt_sample_directions(count: int, reference: torch.Tensor, *, seed: int) -> torch.Tensor:
    ...
    if seed < 0:
        raise ValueError("seed must be non-negative")
    validate_cuda_tensor("reference", reference, dtype=torch.float32, ndim=2)
    directions = _required_native_op("bdpt_sample_directions")(int(count), reference, int(seed))
...
def bdpt_reflection_launch_inputs(
    ...
    *,
    ...
    if sample_count < 0:
        raise ValueError("sample_count must be non-negative")
    exported = _required_native_op("bdpt_reflection_launch_inputs")(tx_positions, int(tx_index), int(sample_count))
    if not isinstance(exported, dict):
        raise TypeError("_channel_native.bdpt_reflection_launch_inputs must return a dict")
    validate_cuda_tensor("ray_o", exported["ray_o"], dtype=torch.float32, ndim=2, trailing_shape=(3,))
    validate_cuda_tensor("ray_tmax", exported["ray_tmax"], dtype=torch.float32, ndim=1)
    validate_cuda_tensor("active", exported["active"], dtype=torch.bool, ndim=1)
    ...
    validate_cuda_tensor("light_seed", exported["light_seed"], dtype=torch.int64, ndim=1)
    return exported


def bdpt_diffraction_state_wi(state_edge_pos: torch.Tensor, state_src: torch.Tensor) -> torch.Tensor:
    ...
    state_wi = _required_native_op("bdpt_diffraction_state_wi")(state_edge_pos, state_src)
    if not isinstance(state_wi, torch.Tensor):
        raise TypeError("_channel_native.bdpt_diffraction_state_wi must return a tensor")
...
def bdpt_selected_edge_indices(selected: torch.Tensor) -> torch.Tensor:
    ...
    if not isinstance(indices, torch.Tensor):
        raise TypeError("_channel_native.bdpt_selected_edge_indices must return a tensor")
...
def bdpt_diffraction_state_pack(
    ...
    if not isinstance(states, tuple) or len(states) != 12:
        raise TypeError("_channel_native.bdpt_diffraction_state_pack must return 12 tensors")
...
def deterministic_diffraction_state_pack(
    ...
    if not isinstance(states, tuple) or len(states) != 12:
        raise TypeError("_channel_native.deterministic_diffraction_state_pack must return 12 tensors")
    validate_cuda_tensor("state_edge_index", states[0], dtype=torch.int32, ndim=1)
    validate_cuda_tensor("state_edge_pos", states[1], dtype=torch.float32, ndim=2, trailing_shape=(3,))
    validate_cuda_tensor("state_edge_dir", states[2], dtype=torch.float32, ndim=2, trailing_shape=(3,))
    validate_cuda_tensor("state_line_min", states[3], dtype=torch.float32, ndim=1)
...
def deterministic_diffraction_state_pack_selected(
    ...
    if not isinstance(states, tuple) or len(states) != 12:
        raise TypeError("_channel_native.deterministic_diffraction_state_pack_selected must return 12 tensors")
    validate_cuda_tensor("state_edge_index", states[0], dtype=torch.int32, ndim=1)
    validate_cuda_tensor("state_edge_pos", states[1], dtype=torch.float32, ndim=2, trailing_shape=(3,))
    validate_cuda_tensor("state_edge_dir", states[2], dtype=torch.float32, ndim=2, trailing_shape=(3,))
    validate_cuda_tensor("state_line_min", states[3], dtype=torch.float32, ndim=1)
...
def bdpt_diffraction_edge_geometry(
    ...
    *,
    ...
    if not isinstance(geometry, tuple) or len(geometry) != 11:
        raise TypeError("_channel_native.bdpt_diffraction_edge_geometry must return 11 tensors")
...
def bdpt_surface_group_edge_candidates(
    ...
    *,
    ...
    if not isinstance(candidates, tuple) or len(candidates) != 2:
        raise TypeError("_channel_native.bdpt_surface_group_edge_candidates must return 2 tensors")
    validate_cuda_tensor("triangle_edge_count", candidates[0], dtype=torch.int32, ndim=1)
    validate_cuda_tensor("triangle_edge_indices", candidates[1], dtype=torch.int32, ndim=2)
    return candidates
...
def bdpt_face_material_tensors_from_host(
    ...
    if not isinstance(exported, dict):
        raise TypeError("_channel_native.bdpt_face_material_tensors_from_host must return a dict")
    validate_cuda_tensor("eps_r", exported["eps_r"], dtype=torch.float32, ndim=1)
    validate_cuda_tensor("sigma_e", exported["sigma_e"], dtype=torch.float32, ndim=1)
    validate_cuda_tensor("mu_r", exported["mu_r"], dtype=torch.float32, ndim=1)
    validate_cuda_tensor("gain", exported["gain"], dtype=torch.float32, ndim=1)
...
def bdpt_visibility_forward(
    ...
    validate_cuda_tensor("start", start, dtype=torch.float32, ndim=2, trailing_shape=(3,))
    ...
    if not isinstance(out, (tuple, list)):
        raise TypeError("_channel_native.bdpt_visibility_forward must return a tensor sequence")
    return tuple(out)
...
def raydn_scene_create(
    ...
    if not isinstance(out, (tuple, list)) or len(out) != 2:
        raise TypeError("_channel_native.raydn_scene_create must return (handle, owner)")
    handle = out[0]
    if not isinstance(handle, int) or handle == 0:
        raise RuntimeError("_channel_native.raydn_scene_create returned an invalid handle")
    return handle, out[1]
...
def raydn_scene_edge_records(handle: int) -> tuple[torch.Tensor, ...]:
    ...
    if not isinstance(out, (tuple, list)):
        raise TypeError("_channel_native.raydn_scene_edge_records must return a tensor sequence")
    return tuple(out)
...
def bdpt_intersect_forward(
    ...
    *,
    ...
    validate_cuda_tensor("ray_o", ray_o, dtype=torch.float32, ndim=2, trailing_shape=(3,))
    validate_cuda_tensor("ray_d", ray_d, dtype=torch.float32, ndim=2, trailing_shape=(3,))
    ...
    if ray_d.get_device() != ray_o.get_device() or ray_tmax.get_device() != ray_o.get_device():
    ...
    for name in ("p", "n", "geo_n", "barycentric"):
        validate_cuda_tensor(name, exported[name], dtype=torch.float32, ndim=2, trailing_shape=(3,))
    validate_cuda_tensor("uv", exported["uv"], dtype=torch.float32, ndim=2, trailing_shape=(2,))
...
def bdpt_reflection_accumulation_forward(*args: object) -> tuple[torch.Tensor, ...]:
    if not args:
        raise TypeError("bdpt_reflection_accumulation_forward requires a RayDN scene handle")
    ...
    if not isinstance(out, (tuple, list)):
        raise TypeError("_channel_native.bdpt_reflection_accumulation_forward must return a tensor sequence")
    return tuple(out)
...
def bdpt_diffraction_discover_edges(*args: object) -> torch.Tensor:
    ...
    if not isinstance(out, torch.Tensor):
        raise TypeError("_channel_native.bdpt_diffraction_discover_edges must return a tensor")
    return out
...
def bdpt_diffraction_discover_edges_counted(*args: object) -> torch.Tensor:
    ...
    if not isinstance(out, torch.Tensor):
        raise TypeError("_channel_native.bdpt_diffraction_discover_edges_counted must return a tensor")
    return out


def bdpt_diffraction_accumulation_forward(*args: object) -> tuple[torch.Tensor, ...]:
    if not args:
        raise TypeError("bdpt_diffraction_accumulation_forward requires a RayDN scene handle")
    native_args = (_raydn_scene_handle_id(args[0]), *args[1:])
    ...
    if not isinstance(out, (tuple, list)):
        raise TypeError("_channel_native.bdpt_diffraction_accumulation_forward must return a tensor sequence")
    return tuple(out)
...
def raydn_reflection_epc_paths_forward(*args: object) -> tuple[torch.Tensor, ...]:
    if not args:
        raise TypeError("raydn_reflection_epc_paths_forward requires a RayDN scene handle")
    native_args = (_raydn_scene_handle_id(args[0]), *args[1:])
    ...
    if not isinstance(out, (tuple, list)):
        raise TypeError("_channel_native.raydn_reflection_epc_paths_forward must return a tensor sequence")
    return tuple(out)


def raydn_diffraction_paths_order1_forward(*args: object) -> tuple[torch.Tensor, ...]:
    if not args:
        raise TypeError("raydn_diffraction_paths_order1_forward requires a RayDN scene handle")
    native_args = (_raydn_scene_handle_id(args[0]), *args[1:])
    ...
    if not isinstance(out, (tuple, list)):
        raise TypeError("_channel_native.raydn_diffraction_paths_order1_forward must return a tensor sequence")
    return tuple(out)
...
def path_los_export(
    ...
    *,
    ...
    exported = _required_native_op("path_los_export")(tx_positions, tx_power, rx_positions, float(frequency_hz))
    if not isinstance(exported, dict):
...
def _validate_deterministic_topology_block(name: str, block: dict[str, torch.Tensor], sequence_width: int) -> None:
...
def _validate_topology_extra_fields(
    ...
    for key, dtype in _DETERMINISTIC_TOPOLOGY_EXTRA_SCHEMA:
        ...
        if key in {"interaction_position", "interaction_normal"}:
            validate_cuda_tensor(f"{name}.{key}", tensor, dtype=dtype, ndim=2, trailing_shape=(3,))
...
def path_concat_vec3(blocks: tuple[torch.Tensor, ...] | list[torch.Tensor]) -> torch.Tensor:
    ...
    for index, block in enumerate(blocks):
        validate_cuda_tensor(f"blocks[{index}]", block, dtype=torch.float32, ndim=2, trailing_shape=(3,))
...
def deterministic_concat_topology_blocks(
    ...
    *,
    ...
    concat_presence = {key: key in blocks[0] for key, _dtype in _DETERMINISTIC_TOPOLOGY_EXTRA_SCHEMA}
    for index, block in enumerate(blocks):
        _validate_path_block(f"blocks[{index}]", block)
        _validate_topology_extra_fields(f"blocks[{index}]", block, int(sequence_width), concat_presence)
    exported = _required_native_op("deterministic_concat_topology_blocks")(tuple(blocks), int(sequence_width))
    if not isinstance(exported, dict):
        raise TypeError("_channel_native.deterministic_concat_topology_blocks must return a dict")
    _validate_path_block("deterministic_concat_topology_blocks", exported)
    _validate_topology_extra_fields("deterministic_concat_topology_blocks", exported, int(sequence_width), concat_presence)
    expected_count = sum(int(block["valid"].shape[0]) for block in blocks)
    if exported["valid"].shape != (expected_count,):
        raise ValueError("_channel_native.deterministic_concat_topology_blocks returned bad path count")
    return exported
...
def deterministic_gather_topology_block(
    ...
    *,
    ...
    field_presence = {key: key in block for key, _dtype in _DETERMINISTIC_TOPOLOGY_EXTRA_SCHEMA}
    ...
    if not isinstance(exported, dict):
        raise TypeError("_channel_native.deterministic_gather_topology_block must return a dict")
    _validate_path_block("deterministic_gather_topology_block", exported)
    _validate_topology_extra_fields("deterministic_gather_topology_block", exported, int(sequence_width), field_presence)
    expected_count = int(order.shape[0]) if max_count < 0 else min(int(order.shape[0]), int(max_count))
    if exported["valid"].shape != (expected_count,):
        raise ValueError("_channel_native.deterministic_gather_topology_block returned bad path count")
    return exported
...
def path_los_visibility_inputs(
    ...
    exported = _required_native_op("path_los_visibility_inputs")(tx_positions, rx_positions, tx_id, rx_id)
    if not isinstance(exported, dict):
    ...
    if exported["active"].shape != tx_id.shape:
        raise ValueError("_channel_native.path_los_visibility_inputs returned bad active shape")
    return exported
...
def path_filter_los(
    ...
    block = _required_native_op("path_filter_los")(tx_id, rx_id, path_length_m, delay_s, path_gain, visible)
...
def deterministic_los_topology_block(
    ...
    *,
    ...
    if not isinstance(block, dict):
        raise TypeError("_channel_native.deterministic_los_topology_block must return a dict")
    _validate_deterministic_topology_block("deterministic_los_topology_block", block, int(sequence_width))
    return block
...
def deterministic_topology_default_fields(reference: torch.Tensor) -> dict[str, torch.Tensor]:
    ...
    if not isinstance(exported, dict):
        raise TypeError("_channel_native.deterministic_topology_default_fields must return a dict")
    validate_cuda_tensor("interaction_position", exported["interaction_position"], dtype=torch.float32, ndim=2, trailing_shape=(3,))
    validate_cuda_tensor("interaction_normal", exported["interaction_normal"], dtype=torch.float32, ndim=2, trailing_shape=(3,))
    validate_cuda_tensor("material_id", exported["material_id"], dtype=torch.int32, ndim=1)
    validate_cuda_tensor("path_field", exported["path_field"], dtype=torch.complex64, ndim=1)
    ...
    if (
        ...
        raise ValueError("_channel_native.deterministic_topology_default_fields returned bad shape")
    return exported
...
def deterministic_pad_topology_sequences(
    *,
    ...
    validate_cuda_tensor("interaction_position", interaction_position, dtype=torch.float32, ndim=2, trailing_shape=(3,))
    validate_cuda_tensor("interaction_normal", interaction_normal, dtype=torch.float32, ndim=2, trailing_shape=(3,))
    validate_cuda_tensor("primitive_sequence", primitive_sequence, dtype=torch.int32, ndim=2)
    validate_cuda_tensor("material_sequence", material_sequence, dtype=torch.int32, ndim=2)
    validate_cuda_tensor("interaction_positions", interaction_positions, dtype=torch.float32, ndim=3)
    validate_cuda_tensor("interaction_normals", interaction_normals, dtype=torch.float32, ndim=3)
    if width < 0:
    ...
    if not isinstance(exported, dict):
        raise TypeError("_channel_native.deterministic_pad_topology_sequences must return a dict")
    validate_cuda_tensor("primitive_sequence", exported["primitive_sequence"], dtype=torch.int32, ndim=2)
    validate_cuda_tensor("material_sequence", exported["material_sequence"], dtype=torch.int32, ndim=2)
    validate_cuda_tensor("interaction_positions", exported["interaction_positions"], dtype=torch.float32, ndim=3)
    validate_cuda_tensor("interaction_normals", exported["interaction_normals"], dtype=torch.float32, ndim=3)
    expected_i32 = (count, int(width))
    expected_vec = (count, int(width), 3)
    if exported["primitive_sequence"].shape != expected_i32 or exported["material_sequence"].shape != expected_i32:
        raise ValueError("_channel_native.deterministic_pad_topology_sequences returned bad sequence shape")
    if (
        exported["interaction_positions"].shape != expected_vec
        or exported["interaction_normals"].shape != expected_vec
    ):
        raise ValueError("_channel_native.deterministic_pad_topology_sequences returned bad interaction shape")
    return exported
...
def deterministic_topology_base_fields(
    *,
    ...
    validate_cuda_tensor("primitive_source", primitive_source, dtype=torch.int32, ndim=1)
    ...
    if not isinstance(block, dict):
        raise TypeError("_channel_native.deterministic_topology_base_fields must return a dict")
    _validate_path_block("deterministic_topology_base_fields", block)
    return block


def deterministic_repeat_range(reference: torch.Tensor, *, start: int, end: int, repeats: int) -> torch.Tensor:
    validate_cuda_tensor("reference", reference, dtype=torch.float32, ndim=1)
    if end < start:
        raise ValueError("end must be greater than or equal to start")
    if repeats <= 0:
        raise ValueError("repeats must be positive")
    out = _required_native_op("deterministic_repeat_range")(reference, int(start), int(end), int(repeats))
    validate_cuda_tensor("out", out, dtype=torch.int32, ndim=1)
    if out.shape != ((int(end) - int(start)) * int(repeats),):
        raise ValueError("_channel_native.deterministic_repeat_range returned bad shape")
    return out
...
def deterministic_face_anchor_points(vertices: torch.Tensor, faces: torch.Tensor) -> torch.Tensor:
    ...
    if not isinstance(out, torch.Tensor):
        raise TypeError("_channel_native.deterministic_face_anchor_points must return a tensor")
    validate_cuda_tensor("out", out, dtype=torch.float32, ndim=2, trailing_shape=(3,))
    if out.shape != (faces.shape[0], 3):
        raise ValueError("_channel_native.deterministic_face_anchor_points returned bad shape")
    return out
...
def deterministic_reflection_epc_input_batch(
    *,
    ...
    validate_cuda_tensor("rx_positions", rx_positions, dtype=torch.float32, ndim=2, trailing_shape=(3,))
    validate_cuda_tensor("tri_a", tri_a, dtype=torch.float32, ndim=2, trailing_shape=(3,))
    validate_cuda_tensor("normals", normals, dtype=torch.float32, ndim=2, trailing_shape=(3,))
    ...
    if not isinstance(exported, dict):
        raise TypeError("_channel_native.deterministic_reflection_epc_input_batch must return a dict")
    expected_fields = {
        "tx_batch",
        "rx_batch",
        "rx_indices",
    ...
    if set(exported) != expected_fields:
        raise ValueError("_channel_native.deterministic_reflection_epc_input_batch returned unexpected fields")
    pair_count = (int(rx_end) - int(rx_start)) * int(sequences.shape[0])
    depth = int(sequences.shape[1])
    validate_cuda_tensor("tx_batch", exported["tx_batch"], dtype=torch.float32, ndim=2, trailing_shape=(3,))
    validate_cuda_tensor("rx_batch", exported["rx_batch"], dtype=torch.float32, ndim=2, trailing_shape=(3,))
    validate_cuda_tensor("rx_indices", exported["rx_indices"], dtype=torch.int32, ndim=1)
    validate_cuda_tensor("sequence_batch", exported["sequence_batch"], dtype=torch.int32, ndim=2)
    validate_cuda_tensor("direct_plane_points", exported["direct_plane_points"], dtype=torch.float32, ndim=3)
    validate_cuda_tensor("direct_plane_normals", exported["direct_plane_normals"], dtype=torch.float32, ndim=3)
    if exported["tx_batch"].shape != (pair_count, 3):
        raise ValueError("_channel_native.deterministic_reflection_epc_input_batch returned bad tx_batch shape")
    if exported["rx_batch"].shape != (pair_count, 3):
        raise ValueError("_channel_native.deterministic_reflection_epc_input_batch returned bad rx_batch shape")
    if exported["rx_indices"].shape != (pair_count,):
        raise ValueError("_channel_native.deterministic_reflection_epc_input_batch returned bad rx_indices shape")
    if exported["sequence_batch"].shape != (pair_count, depth):
        raise ValueError("_channel_native.deterministic_reflection_epc_input_batch returned bad sequence_batch shape")
    if exported["direct_plane_points"].shape != (pair_count, depth, 3):
        raise ValueError("_channel_native.deterministic_reflection_epc_input_batch returned bad direct_plane_points shape")
    if exported["direct_plane_normals"].shape != (pair_count, depth, 3):
        raise ValueError("_channel_native.deterministic_reflection_epc_input_batch returned bad direct_plane_normals shape")
    return exported
...
def deterministic_face_sequence_chunk(
    ...
    *,
    ...
    if not isinstance(out, torch.Tensor):
        raise TypeError("_channel_native.deterministic_face_sequence_chunk must return a tensor")
    validate_cuda_tensor("out", out, dtype=torch.int32, ndim=2)
    if out.shape != (int(end) - int(start), int(depth)):
        raise ValueError("_channel_native.deterministic_face_sequence_chunk returned bad shape")
    return out
...
def deterministic_mapped_face_sequence_chunk(
    ...
    *,
    ...
    if not isinstance(out, torch.Tensor):
        raise TypeError("_channel_native.deterministic_mapped_face_sequence_chunk must return a tensor")
    validate_cuda_tensor("out", out, dtype=torch.int32, ndim=2)
    if out.shape != (int(end) - int(start), int(depth)):
        raise ValueError("_channel_native.deterministic_mapped_face_sequence_chunk returned bad shape")
    return out
...
def deterministic_reflection_order1_compact(
    *,
    ...
    if not epc_faces.is_cuda or epc_faces.ndim != 2 or not epc_faces.is_contiguous():
        raise ValueError("epc_faces must be a contiguous CUDA tensor with shape (N, depth)")
    ...
    if sequence_batch.shape[0] != count or sequence_batch.shape[1] < 1:
        raise ValueError("sequence_batch must match visible and include the first bounce")
    ...
    if not isinstance(exported, dict):
        raise TypeError("_channel_native.deterministic_reflection_order1_compact must return a dict")
    expected_fields = {
        "selected_faces",
        "selected_points",
        "selected_normals",
    ...
    if set(exported) != expected_fields:
        raise ValueError("_channel_native.deterministic_reflection_order1_compact returned unexpected fields")
    selected_count = int(exported["selected_faces"].shape[0])
    validate_cuda_tensor("selected_faces", exported["selected_faces"], dtype=torch.int32, ndim=1)
    validate_cuda_tensor("selected_points", exported["selected_points"], dtype=torch.float32, ndim=2, trailing_shape=(3,))
    validate_cuda_tensor("selected_normals", exported["selected_normals"], dtype=torch.float32, ndim=2, trailing_shape=(3,))
    validate_cuda_tensor("selected_rx_id", exported["selected_rx_id"], dtype=torch.int32, ndim=1)
    validate_cuda_tensor("tx_keep", exported["tx_keep"], dtype=torch.float32, ndim=2, trailing_shape=(3,))
    validate_cuda_tensor("rx_keep", exported["rx_keep"], dtype=torch.float32, ndim=2, trailing_shape=(3,))
    ...
    for key in expected_fields - {"selected_faces"}:
        if exported[key].shape[0] != selected_count:
            raise ValueError(f"_channel_native.deterministic_reflection_order1_compact returned bad {key} shape")
    return exported
...
def deterministic_reflection_sequence_compact(
    *,
    ...
    if not epc_sequences.is_cuda or epc_sequences.ndim != 2 or not epc_sequences.is_contiguous():
        raise ValueError("epc_sequences must be a contiguous CUDA tensor with shape (N, depth)")
    validate_cuda_tensor("epc_hits", epc_hits, dtype=torch.float32, ndim=3)
    validate_cuda_tensor("epc_normals", epc_normals, dtype=torch.float32, ndim=3)
    validate_cuda_tensor("rx_indices", rx_indices, dtype=torch.int32, ndim=1)
    validate_cuda_tensor("tx", tx, dtype=torch.float32, ndim=1)
    ...
    if not isinstance(exported, dict):
        raise TypeError("_channel_native.deterministic_reflection_sequence_compact must return a dict")
    expected_fields = {
        "selected_sequences",
        "selected_hits",
        "selected_normals",
    ...
    if set(exported) != expected_fields:
        raise ValueError("_channel_native.deterministic_reflection_sequence_compact returned unexpected fields")
    selected_count = int(exported["selected_sequences"].shape[0])
    validate_cuda_tensor("selected_sequences", exported["selected_sequences"], dtype=torch.int32, ndim=2)
    validate_cuda_tensor("selected_hits", exported["selected_hits"], dtype=torch.float32, ndim=3)
    validate_cuda_tensor("selected_normals", exported["selected_normals"], dtype=torch.float32, ndim=3)
    validate_cuda_tensor("selected_rx_id", exported["selected_rx_id"], dtype=torch.int32, ndim=1)
    validate_cuda_tensor("selected_tx", exported["selected_tx"], dtype=torch.float32, ndim=2, trailing_shape=(3,))
    validate_cuda_tensor("selected_rx", exported["selected_rx"], dtype=torch.float32, ndim=2, trailing_shape=(3,))
    validate_cuda_tensor("tx_power", exported["tx_power"], dtype=torch.float32, ndim=1)
    ...
    validate_cuda_tensor("first_face", exported["first_face"], dtype=torch.int32, ndim=1)
    validate_cuda_tensor("material_id", exported["material_id"], dtype=torch.int32, ndim=1)
    validate_cuda_tensor("material_sequence", exported["material_sequence"], dtype=torch.int32, ndim=2)
    validate_cuda_tensor("first_hit", exported["first_hit"], dtype=torch.float32, ndim=2, trailing_shape=(3,))
    validate_cuda_tensor("first_normal", exported["first_normal"], dtype=torch.float32, ndim=2, trailing_shape=(3,))
    expected_shapes = {
    ...
    for key, shape in expected_shapes.items():
        if tuple(exported[key].shape) != shape:
            raise ValueError(f"_channel_native.deterministic_reflection_sequence_compact returned bad {key} shape")
    return exported
...
def deterministic_diffraction_order1_compact(
    *,
    ...
    if not isinstance(exported, dict):
        raise TypeError("_channel_native.deterministic_diffraction_order1_compact must return a dict")
    expected_fields = {
        "rx_id",
        "depth",
        "edge_id",
    ...
    if set(exported) != expected_fields:
        raise ValueError("_channel_native.deterministic_diffraction_order1_compact returned unexpected fields")
    ...
    for key in expected_fields - {"interaction_position"}:
        if exported[key].shape != (selected_count,):
            raise ValueError(f"_channel_native.deterministic_diffraction_order1_compact returned bad {key} shape")
    if exported["interaction_position"].shape != (selected_count, 3):
        raise ValueError("_channel_native.deterministic_diffraction_order1_compact returned bad interaction_position shape")
    return exported


def deterministic_normalize_vec3(values: torch.Tensor, *, eps: float = 1.0e-6) -> torch.Tensor:
    validate_cuda_tensor("values", values, dtype=torch.float32, ndim=2, trailing_shape=(3,))
    if eps <= 0.0:
        raise ValueError("eps must be positive")
    out = _required_native_op("deterministic_normalize_vec3")(values, float(eps))
    if not isinstance(out, torch.Tensor):
        raise TypeError("_channel_native.deterministic_normalize_vec3 must return a tensor")
    validate_cuda_tensor("out", out, dtype=torch.float32, ndim=2, trailing_shape=(3,))
    if out.shape != values.shape:
        raise ValueError("_channel_native.deterministic_normalize_vec3 returned bad shape")
    return out
...
def deterministic_reflect_points(
    ...
    validate_cuda_tensor("points", points, dtype=torch.float32, ndim=2, trailing_shape=(3,))
    validate_cuda_tensor("plane_points", plane_points, dtype=torch.float32, ndim=2, trailing_shape=(3,))
    validate_cuda_tensor("normals", normals, dtype=torch.float32, ndim=2, trailing_shape=(3,))
    ...
    out = _required_native_op("deterministic_reflect_points")(points, plane_points, normals)
    if not isinstance(out, torch.Tensor):
        raise TypeError("_channel_native.deterministic_reflect_points must return a tensor")
    validate_cuda_tensor("out", out, dtype=torch.float32, ndim=2, trailing_shape=(3,))
    if out.shape != points.shape:
        raise ValueError("_channel_native.deterministic_reflect_points returned bad shape")
    return out
...
def deterministic_face_groups(
    ...
    *,
    ...
    if set(exported) != expected_fields:
        raise ValueError("_channel_native.deterministic_face_groups returned unexpected fields")
    face_count = int(tri_a.shape[0])
    validate_cuda_tensor("face_group_id", exported["face_group_id"], dtype=torch.int32, ndim=1)
    validate_cuda_tensor("representative_faces", exported["representative_faces"], dtype=torch.int64, ndim=1)
    validate_cuda_tensor("surface_group_id", exported["surface_group_id"], dtype=torch.int32, ndim=1)
    validate_cuda_tensor("surface_group_size", exported["surface_group_size"], dtype=torch.int32, ndim=1)
    validate_cuda_tensor("surface_group_members", exported["surface_group_members"], dtype=torch.int32, ndim=1)
    group_count = exported["group_count"]
    if not isinstance(group_count, int):
        raise TypeError("_channel_native.deterministic_face_groups returned non-int group_count")
    if exported["face_group_id"].shape != (face_count,) or exported["surface_group_id"].shape != (face_count,):
        raise ValueError("_channel_native.deterministic_face_groups returned bad face group shape")
    if (
        exported["representative_faces"].shape != (group_count,)
        or exported["surface_group_size"].shape != (group_count,)
    ):
        raise ValueError("_channel_native.deterministic_face_groups returned bad group shape")
    return exported
...
def deterministic_surface_face_groups(surface_ids: torch.Tensor) -> dict[str, torch.Tensor | int]:
    ...
    if not isinstance(exported, dict):
        raise TypeError("_channel_native.deterministic_surface_face_groups must return a dict")
    expected_fields = {
        "face_group_id",
        "representative_faces",
        "surface_group_id",
    ...
    if set(exported) != expected_fields:
        raise ValueError("_channel_native.deterministic_surface_face_groups returned unexpected fields")
    ...
    group_count = exported["group_count"]
    if not isinstance(group_count, int):
        raise TypeError("_channel_native.deterministic_surface_face_groups returned non-int group_count")
    if exported["face_group_id"].shape != (face_count,) or exported["surface_group_id"].shape != (face_count,):
        raise ValueError("_channel_native.deterministic_surface_face_groups returned bad face group shape")
    if (
        exported["representative_faces"].shape != (group_count,)
        or exported["surface_group_size"].shape != (group_count,)
    ):
        raise ValueError("_channel_native.deterministic_surface_face_groups returned bad group shape")
    return exported
...
def _validate_path_reflection_candidates(name: str, candidates: dict[str, torch.Tensor]) -> None:
    ...
    for key in ("seg0_start", "seg0_end", "seg1_start", "seg1_end"):
        ...
        if not isinstance(tensor, torch.Tensor):
            raise TypeError(f"{name}.{key} must be a torch.Tensor")
        validate_cuda_tensor(f"{name}.{key}", tensor, dtype=torch.float32, ndim=2, trailing_shape=(3,))
...
def path_diffraction_block(
    ...
    *,
    ...
    if not isinstance(raydn_output, tuple) or len(raydn_output) != 18:
        raise TypeError("raydn_output must be the 18-tensor RayDN diffraction path tuple")
    for index in (1, 3, 4, 5):
        validate_cuda_tensor(f"raydn_output[{index}]", raydn_output[index], dtype=torch.int32 if index != 1 else torch.bool, ndim=1)
    for index in (8, 9, 10, 11, 12, 13, 14):
        validate_cuda_tensor(f"raydn_output[{index}]", raydn_output[index], dtype=torch.float32, ndim=1)
...
def path_diffraction_paths_order1(
    ...
    *,
    ...
    if len(edge_geometry) != 11:
        raise TypeError("edge_geometry must contain the 11-tensor diffraction edge geometry tuple")
    ...
    if not isinstance(out, dict):
        raise TypeError("_channel_native.path_diffraction_paths_order1 must return a dict")
...
def path_merge_blocks(
    ...
    *,
    ...
    out = _required_native_op("path_merge_blocks")(tuple(blocks), int(tx_count), int(max_depth))
    if not isinstance(out, dict):
...
def mc_point_component_power(path_gain: torch.Tensor, *, include_los: bool) -> dict[str, torch.Tensor]:
    validate_cuda_tensor("path_gain", path_gain, dtype=torch.float32, ndim=2)
    exported = _required_native_op("mc_point_component_power")(path_gain, bool(include_los))
    if not isinstance(exported, dict):
...
def mc_los_component_maps_from_matrix(los: torch.Tensor, *, rows: int, cols: int) -> torch.Tensor:
    ...
    if los.shape[1] != rows * cols:
        raise ValueError("los columns must match rows * cols")
    maps = _required_native_op("mc_los_component_maps_from_matrix")(los, int(rows), int(cols))
    if not isinstance(maps, torch.Tensor):
        raise TypeError("_channel_native.mc_los_component_maps_from_matrix must return a tensor")
    validate_cuda_tensor("maps", maps, dtype=torch.float32, ndim=3)
    if maps.shape != (los.shape[0], cols, rows):
        raise ValueError("_channel_native.mc_los_component_maps_from_matrix returned an unexpected shape")
    return maps
...
def deterministic_reflection_field(
    ...
    *,
    ...
    validate_cuda_tensor("delay_s", exported["delay_s"], dtype=torch.float32, ndim=1)
    if exported["path_length_m"].shape != (count,) or exported["delay_s"].shape != (count,):
        raise ValueError("_channel_native.deterministic_reflection_field returned bad length shape")
    return exported
...
def deterministic_reflection_sequence_field(
    ...
    *,
    ...
    validate_cuda_tensor("delay_s", exported["delay_s"], dtype=torch.float32, ndim=1)
    if exported["path_length_m"].shape != (count,) or exported["delay_s"].shape != (count,):
        raise ValueError("_channel_native.deterministic_reflection_sequence_field returned bad length shape")
    return exported
...
def deterministic_delay_to_path_length(delay_s: torch.Tensor) -> torch.Tensor:
    ...
    if native is None or not hasattr(native, "deterministic_delay_to_path_length"):
        raise RuntimeError("_channel_native.deterministic_delay_to_path_length CUDA kernel is required")
    ...
    if path_length.shape != delay_s.shape:
        raise ValueError("_channel_native.deterministic_delay_to_path_length returned bad shape")
    return path_length
...
def deterministic_pack_complex(field_real: torch.Tensor, field_imag: torch.Tensor) -> torch.Tensor:
    ...
    if native is None or not hasattr(native, "deterministic_pack_complex"):
        raise RuntimeError("_channel_native.deterministic_pack_complex CUDA kernel is required")
    ...
    if field.shape != field_real.shape:
        raise ValueError("_channel_native.deterministic_pack_complex returned bad shape")
    return field


def deterministic_phase_from_field(field_real: torch.Tensor, field_imag: torch.Tensor) -> torch.Tensor:
    validate_cuda_tensor("field_real", field_real, dtype=torch.float32, ndim=1)
    validate_cuda_tensor("field_imag", field_imag, dtype=torch.float32, ndim=1)
    if field_imag.shape != field_real.shape:
        raise ValueError("field_imag must match field_real")
    ...
    if native is None or not hasattr(native, "deterministic_phase_from_field"):
        raise RuntimeError("_channel_native.deterministic_phase_from_field CUDA kernel is required")
    ...
    if phase.shape != field_real.shape:
        raise ValueError("_channel_native.deterministic_phase_from_field returned bad shape")
    return phase
...
def deterministic_zero_field_phase(reference: torch.Tensor) -> dict[str, torch.Tensor]:
    ...
    if native is None or not hasattr(native, "deterministic_zero_field_phase"):
        raise RuntimeError("_channel_native.deterministic_zero_field_phase CUDA kernel is required")
    exported = native.deterministic_zero_field_phase(reference)
    if not isinstance(exported, dict):
        raise TypeError("_channel_native.deterministic_zero_field_phase must return a dict")
    validate_cuda_tensor("path_field", exported["path_field"], dtype=torch.complex64, ndim=1)
    validate_cuda_tensor("phase_rad", exported["phase_rad"], dtype=torch.float32, ndim=1)
    if exported["path_field"].shape != reference.shape or exported["phase_rad"].shape != reference.shape:
        raise ValueError("_channel_native.deterministic_zero_field_phase returned bad shape")
    return exported


def deterministic_phase_from_length(path_length_m: torch.Tensor, *, frequency_hz: float) -> torch.Tensor:
    validate_cuda_tensor("path_length_m", path_length_m, dtype=torch.float32, ndim=1)
    if frequency_hz <= 0.0:
        raise ValueError("frequency_hz must be positive")
    native = native_extension()
    if native is None or not hasattr(native, "deterministic_phase_from_length"):
        raise RuntimeError("_channel_native.deterministic_phase_from_length CUDA kernel is required")
    phase = native.deterministic_phase_from_length(path_length_m, float(frequency_hz))
    validate_cuda_tensor("phase_rad", phase, dtype=torch.float32, ndim=1)
    if phase.shape != path_length_m.shape:
        raise ValueError("_channel_native.deterministic_phase_from_length returned bad shape")
    return phase


def deterministic_field_from_power_phase(path_gain: torch.Tensor, phase_rad: torch.Tensor) -> dict[str, torch.Tensor]:
    validate_cuda_tensor("path_gain", path_gain, dtype=torch.float32, ndim=1)
    ...
    if native is None or not hasattr(native, "deterministic_field_from_power_phase"):
        raise RuntimeError("_channel_native.deterministic_field_from_power_phase CUDA kernel is required")
    exported = native.deterministic_field_from_power_phase(path_gain, phase_rad)
    if not isinstance(exported, dict):
        raise TypeError("_channel_native.deterministic_field_from_power_phase must return a dict")
    validate_cuda_tensor("field_real", exported["field_real"], dtype=torch.float32, ndim=1)
    validate_cuda_tensor("field_imag", exported["field_imag"], dtype=torch.float32, ndim=1)
    if exported["field_real"].shape != path_gain.shape or exported["field_imag"].shape != path_gain.shape:
        raise ValueError("_channel_native.deterministic_field_from_power_phase returned bad shape")
    return exported
