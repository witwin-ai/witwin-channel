...
class _FieldCoupledDdAdFunction(torch.autograd.Function):
    ...
    def backward(
        ...
        need_geometry = any(bool(ctx.needs_input_grad[index]) for index in (0, 1))
        need_eps = any(
            bool(ctx.needs_input_grad[index]) for index in (15, 20, 25, 30)
        )
        need_sigma = any(
            bool(ctx.needs_input_grad[index]) for index in (16, 21, 26, 31)
        )
        need_gain = any(
            bool(ctx.needs_input_grad[index]) for index in (18, 23, 28, 33)
        )
        need_thickness = any(
            bool(ctx.needs_input_grad[index]) for index in (19, 24, 29, 34)
        )
        need_frequency = bool(ctx.needs_input_grad[35])
        grads = (grad_field_vector, grad_coefficient, grad_path_field, grad_path_gain)
        if not (
            need_geometry
            or need_eps
