...
_PACKAGE_MODULE = "witwin.channel_native._channel_native"
...
def _assert_packaged_origin(module: object) -> None:
    ...
    if not origin.is_relative_to(package_dir):
        raise ExtensionLoadError(
            f"refusing _channel_native resolved outside witwin.channel_native: {origin}"
        )
...
def _load_packaged_extension() -> object:
    module = importlib.import_module(
        "._channel_native", package="witwin.channel_native"
    )
...
def _load_native_extension() -> object:
    ...
    raise ExtensionLoadError(
        "witwin.channel_native._channel_native is not installed; a development "
