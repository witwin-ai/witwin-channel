...
def _scene(
    ...
    *,
    ...
    return Scene(
        structures=[wall],
        transmitters=[Transmitter(position=tx_pos)],
        receivers=receivers or [ReceiverPoint(position=rx_pos)],
...
def _smooth_reflection_power() -> float:
    result = solve(_scene(0.0), Config(max_depth=1, components={"reflection"}))
...
def test_rough_wall_energy_budget_and_smooth_limit():
    ...
    tiny = solve(_scene(5.0e-5, 0.3), _config())
...
def test_narrow_lobe_normalization_cross_check():
    ...
    smooth_small = solve(
        _scene(0.0, 0.3, half_size=1.0), Config(max_depth=1, components={"reflection"})
    )
...
def test_table_passivity_per_incidence_angle():
    ...
    rough = Roughness(rms_height_m=0.015, corr_length_x_m=0.15, corr_length_y_m=0.15)
...
def test_realization_coherent_flat_screen_matches_smooth_reflection():
    ...
    flat = solve(
        _scene(0.0, half_size=1.0, phase_screen=_screen(0, flat=True)), _config()
    )
...
def test_realization_coherent_reproducible_and_realization_dependent():
    ...
    other = solve(_scene(0.008, half_size=1.0, phase_screen=_screen(2)), cfg)
