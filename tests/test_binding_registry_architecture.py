...
BINDING_ROOT = REPOSITORY_ROOT / "native/channel/binding"
...
def test_binding_module_is_the_unique_pybind_module_owner() -> None:
    module_owners = [
        ...
        for path in (REPOSITORY_ROOT / "native/channel").rglob("*.cpp")
        if "PYBIND11_MODULE" in path.read_text(encoding="utf-8")
    ]

    assert module_owners == ["native/channel/binding/module.cpp"]
...
def test_binding_module_only_sets_doc_and_calls_registrars() -> None:
    ...
    assert statements.count(
        'module.doc() = "Channel Torch/CUDA extension."'
    ) == 1
    registrar_calls = statements.copy()
    registrar_calls.remove('module.doc() = "Channel Torch/CUDA extension."')
...
def test_binding_registries_are_declared_and_built_without_legacy_aggregators() -> None:
    ...
    for registry in EXPECTED_REGISTRIES:
        ...
        assert f"native/channel/binding/{registry}.cpp" in cmake
    ...
    for legacy_source in (
        "native/channel/bindings.cpp",
        "native/channel/binding/all.cpp",
    ):
