...
void register_packed_state_bindings(nb::module_ &m) {
    ...
    m.def(
        ...
        [](
            ...
            DiffFloat f0_mu_r = drjit::zeros<DiffFloat>(count);
            ...
            DiffFloat f1_mu_r = drjit::zeros<DiffFloat>(count);
            ...
            push_core(f0_eta_r); push_core(f0_mu_r); push_core(f0_sigma); push_core(f0_gain);
            push_core(f0_use_fresnel);
            push_core(f1_eta_r); push_core(f1_mu_r); push_core(f1_sigma); push_core(f1_gain);
