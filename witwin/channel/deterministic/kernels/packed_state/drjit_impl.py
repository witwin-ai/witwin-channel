...
This path keeps path replay lineage as optional cold state.
"""
...
from witwin.channel.core.numerics.arrays import concat_arrays, zeros_point3, zeros_vector3
...
def build_diffraction_path_slots(
    *,
    ...
    object_slots = None
    if return_geometry:
        vertex_slots = [zeros_point3(count) for _ in range(max_depth)]
        normal_slots = [zeros_vector3(count) for _ in range(max_depth)]
...
def gather_state_arrays(state_arrays: dict, indices) -> dict:
    gathered = {
        ...
        SK_FACE0_MU_R: dr.gather(wt.Float, state_arrays[SK_FACE0_MU_R], indices),
        ...
        SK_FACE1_MU_R: dr.gather(wt.Float, state_arrays[SK_FACE1_MU_R], indices),
...
def concat_state_arrays(state_arrays_list: list[dict]) -> dict:
    ...
    if len(non_empty) == 0:
        ...
        return State.empty(history_size=history_size, retain_lineage_state=retain_cold)
    if len(non_empty) == 1:
        return non_empty[0]

    history_size = max(Geo.history_size(state_arrays) for state_arrays in non_empty)
    state_arrays = {
        SK_EDGE_IDX: concat_arrays(wt.UInt32, [state[SK_EDGE_IDX] for state in non_empty]),
        SK_EDGE_POS: wt.Point3f(
            concat_arrays(wt.Float, [state[SK_EDGE_POS].x for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_EDGE_POS].y for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_EDGE_POS].z for state in non_empty]),
        ),
        SK_EDGE_DIR: wt.Vector3f(
            concat_arrays(wt.Float, [state[SK_EDGE_DIR].x for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_EDGE_DIR].y for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_EDGE_DIR].z for state in non_empty]),
        ),
        SK_N0: wt.Vector3f(
            concat_arrays(wt.Float, [state[SK_N0].x for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_N0].y for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_N0].z for state in non_empty]),
        ),
        SK_NN: wt.Vector3f(
            concat_arrays(wt.Float, [state[SK_NN].x for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_NN].y for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_NN].z for state in non_empty]),
        ),
        SK_WEDGE_N: concat_arrays(wt.Float, [state[SK_WEDGE_N] for state in non_empty]),
        SK_ADJACENT_FACE0: concat_arrays(wt.Int32, [state[SK_ADJACENT_FACE0] for state in non_empty]),
        SK_ADJACENT_FACE1: concat_arrays(wt.Int32, [state[SK_ADJACENT_FACE1] for state in non_empty]),
        SK_SOURCE_POS: wt.Point3f(
            concat_arrays(wt.Float, [state[SK_SOURCE_POS].x for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_SOURCE_POS].y for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_SOURCE_POS].z for state in non_empty]),
        ),
        SK_INCIDENT_FIELD: wt.Complex2f(
            concat_arrays(wt.Float, [state[SK_INCIDENT_FIELD].real for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_INCIDENT_FIELD].imag for state in non_empty]),
        ),
        SK_INCIDENT_NORMAL_DERIVATIVE: wt.Complex2f(
            concat_arrays(wt.Float, [state[SK_INCIDENT_NORMAL_DERIVATIVE].real for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_INCIDENT_NORMAL_DERIVATIVE].imag for state in non_empty]),
        ),
        SK_INCIDENT_JONES_U: wt.Complex2f(
            concat_arrays(wt.Float, [state[SK_INCIDENT_JONES_U].real for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_INCIDENT_JONES_U].imag for state in non_empty]),
        ),
        SK_INCIDENT_JONES_V: wt.Complex2f(
            concat_arrays(wt.Float, [state[SK_INCIDENT_JONES_V].real for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_INCIDENT_JONES_V].imag for state in non_empty]),
        ),
        SK_INCIDENT_DERIVATIVE_JONES_U: wt.Complex2f(
            concat_arrays(wt.Float, [state[SK_INCIDENT_DERIVATIVE_JONES_U].real for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_INCIDENT_DERIVATIVE_JONES_U].imag for state in non_empty]),
        ),
        SK_INCIDENT_DERIVATIVE_JONES_V: wt.Complex2f(
            concat_arrays(wt.Float, [state[SK_INCIDENT_DERIVATIVE_JONES_V].real for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_INCIDENT_DERIVATIVE_JONES_V].imag for state in non_empty]),
        ),
        SK_R0: wt.Complex2f(
            concat_arrays(wt.Float, [state[SK_R0].real for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_R0].imag for state in non_empty]),
        ),
        SK_RN: wt.Complex2f(
            concat_arrays(wt.Float, [state[SK_RN].real for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_RN].imag for state in non_empty]),
        ),
        SK_INCIDENT_BASIS_U: wt.Vector3f(
            concat_arrays(wt.Float, [state[SK_INCIDENT_BASIS_U].x for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_INCIDENT_BASIS_U].y for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_INCIDENT_BASIS_U].z for state in non_empty]),
        ),
        SK_INCIDENT_BASIS_V: wt.Vector3f(
            concat_arrays(wt.Float, [state[SK_INCIDENT_BASIS_V].x for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_INCIDENT_BASIS_V].y for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_INCIDENT_BASIS_V].z for state in non_empty]),
        ),
        SK_INCIDENT_BASIS_K: wt.Vector3f(
            concat_arrays(wt.Float, [state[SK_INCIDENT_BASIS_K].x for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_INCIDENT_BASIS_K].y for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_INCIDENT_BASIS_K].z for state in non_empty]),
        ),
        SK_FACE0_OPERATOR_M00: wt.Complex2f(
            concat_arrays(wt.Float, [state[SK_FACE0_OPERATOR_M00].real for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_FACE0_OPERATOR_M00].imag for state in non_empty]),
        ),
        SK_FACE0_OPERATOR_M01: wt.Complex2f(
            concat_arrays(wt.Float, [state[SK_FACE0_OPERATOR_M01].real for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_FACE0_OPERATOR_M01].imag for state in non_empty]),
        ),
        SK_FACE0_OPERATOR_M10: wt.Complex2f(
            concat_arrays(wt.Float, [state[SK_FACE0_OPERATOR_M10].real for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_FACE0_OPERATOR_M10].imag for state in non_empty]),
        ),
        SK_FACE0_OPERATOR_M11: wt.Complex2f(
            concat_arrays(wt.Float, [state[SK_FACE0_OPERATOR_M11].real for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_FACE0_OPERATOR_M11].imag for state in non_empty]),
        ),
        SK_FACE1_OPERATOR_M00: wt.Complex2f(
            concat_arrays(wt.Float, [state[SK_FACE1_OPERATOR_M00].real for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_FACE1_OPERATOR_M00].imag for state in non_empty]),
        ),
        SK_FACE1_OPERATOR_M01: wt.Complex2f(
            concat_arrays(wt.Float, [state[SK_FACE1_OPERATOR_M01].real for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_FACE1_OPERATOR_M01].imag for state in non_empty]),
        ),
        SK_FACE1_OPERATOR_M10: wt.Complex2f(
            concat_arrays(wt.Float, [state[SK_FACE1_OPERATOR_M10].real for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_FACE1_OPERATOR_M10].imag for state in non_empty]),
        ),
        SK_FACE1_OPERATOR_M11: wt.Complex2f(
            concat_arrays(wt.Float, [state[SK_FACE1_OPERATOR_M11].real for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_FACE1_OPERATOR_M11].imag for state in non_empty]),
        ),
        SK_FACE0_ETA_R: concat_arrays(wt.Float, [state[SK_FACE0_ETA_R] for state in non_empty]),
        SK_FACE0_MU_R: concat_arrays(wt.Float, [state[SK_FACE0_MU_R] for state in non_empty]),
        SK_FACE0_SIGMA: concat_arrays(wt.Float, [state[SK_FACE0_SIGMA] for state in non_empty]),
        SK_FACE0_GAIN: concat_arrays(wt.Float, [state[SK_FACE0_GAIN] for state in non_empty]),
        SK_FACE0_USE_FRESNEL: concat_arrays(wt.Bool, [state[SK_FACE0_USE_FRESNEL] for state in non_empty]),
        SK_FACE1_ETA_R: concat_arrays(wt.Float, [state[SK_FACE1_ETA_R] for state in non_empty]),
        SK_FACE1_MU_R: concat_arrays(wt.Float, [state[SK_FACE1_MU_R] for state in non_empty]),
        SK_FACE1_SIGMA: concat_arrays(wt.Float, [state[SK_FACE1_SIGMA] for state in non_empty]),
        SK_FACE1_GAIN: concat_arrays(wt.Float, [state[SK_FACE1_GAIN] for state in non_empty]),
        SK_FACE1_USE_FRESNEL: concat_arrays(wt.Bool, [state[SK_FACE1_USE_FRESNEL] for state in non_empty]),
        SK_INCIDENT_VECTOR_X: wt.Complex2f(
            concat_arrays(wt.Float, [state[SK_INCIDENT_VECTOR_X].real for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_INCIDENT_VECTOR_X].imag for state in non_empty]),
        ),
        SK_INCIDENT_VECTOR_Y: wt.Complex2f(
            concat_arrays(wt.Float, [state[SK_INCIDENT_VECTOR_Y].real for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_INCIDENT_VECTOR_Y].imag for state in non_empty]),
        ),
        SK_INCIDENT_VECTOR_Z: wt.Complex2f(
            concat_arrays(wt.Float, [state[SK_INCIDENT_VECTOR_Z].real for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_INCIDENT_VECTOR_Z].imag for state in non_empty]),
        ),
        SK_INCIDENT_NORMAL_DERIVATIVE_VECTOR_X: wt.Complex2f(
            concat_arrays(wt.Float, [state[SK_INCIDENT_NORMAL_DERIVATIVE_VECTOR_X].real for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_INCIDENT_NORMAL_DERIVATIVE_VECTOR_X].imag for state in non_empty]),
        ),
        SK_INCIDENT_NORMAL_DERIVATIVE_VECTOR_Y: wt.Complex2f(
            concat_arrays(wt.Float, [state[SK_INCIDENT_NORMAL_DERIVATIVE_VECTOR_Y].real for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_INCIDENT_NORMAL_DERIVATIVE_VECTOR_Y].imag for state in non_empty]),
        ),
        SK_INCIDENT_NORMAL_DERIVATIVE_VECTOR_Z: wt.Complex2f(
            concat_arrays(wt.Float, [state[SK_INCIDENT_NORMAL_DERIVATIVE_VECTOR_Z].real for state in non_empty]),
            concat_arrays(wt.Float, [state[SK_INCIDENT_NORMAL_DERIVATIVE_VECTOR_Z].imag for state in non_empty]),
        ),
        SK_PREFIX_REFLECTION_DEPTH: concat_arrays(wt.UInt32, [state[SK_PREFIX_REFLECTION_DEPTH] for state in non_empty]),
        SK_INTERMEDIATE_REFLECTION_DEPTH: concat_arrays(
            wt.UInt32,
            [state[SK_INTERMEDIATE_REFLECTION_DEPTH] for state in non_empty],
        ),
        SK_SUFFIX_REFLECTION_DEPTH: concat_arrays(wt.UInt32, [state[SK_SUFFIX_REFLECTION_DEPTH] for state in non_empty]),
        SK_ORDER: concat_arrays(wt.UInt32, [state[SK_ORDER] for state in non_empty]),
