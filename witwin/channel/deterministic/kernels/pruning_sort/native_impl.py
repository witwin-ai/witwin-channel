...
from witwin.channel.deterministic.kernels.pruning_sort.drjit_impl import (
    _state_pruning_metric,
)
from witwin.channel.deterministic.diffraction.state import Geo, State
...
def _prepare_native_pruning_inputs(state_arrays: dict):
    ...
    materialized_edge_history, materialized_reflection_history = State.history(
        state_arrays
    )
    path_edge_history = [
...
def prune_state_arrays_by_budget(
    ...
    if state_arrays is None or n_states == 0:
        return State.empty(history_size=history_size), report
    ...
    pruned = State.take(state_arrays, keep_idx)
...
def prune_state_arrays_by_budget_pair(
    ...
    *,
    ...
    if state_arrays is None or n_states == 0:
        empty = State.empty(history_size=history_size)
