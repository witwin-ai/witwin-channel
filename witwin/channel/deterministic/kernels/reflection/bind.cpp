...
struct FWeightReflectionOpArrays {
    DiffFloat image_source_x;
    DiffFloat image_source_y;
    DiffFloat image_source_z;
    DiffFloat slot_plane_point_x;
    ...
    Float slot_sigma;
    Float slot_gain;
    Int32 transition_support_valid;
    Int32 transition_primary_side;
    DiffFloat transition_edge_distance;
    DiffFloat transition_edge_v0_x;
    DiffFloat transition_edge_v0_y;
    DiffFloat transition_edge_v0_z;
    DiffFloat transition_edge_v1_x;
    DiffFloat transition_edge_v1_y;
    DiffFloat transition_edge_v1_z;
    Int32 adjacent_valid;
    DiffFloat adjacent_plane_point_x;
    DiffFloat adjacent_plane_point_y;
    DiffFloat adjacent_plane_point_z;
    DiffFloat adjacent_plane_normal_x;
    DiffFloat adjacent_plane_normal_y;
    DiffFloat adjacent_plane_normal_z;
    Float adjacent_eta_r;
    Float adjacent_mu_r;
    Float adjacent_sigma;
    Float adjacent_gain;
    DiffFloat rx_x;
    DiffFloat rx_y;
    DiffFloat rx_z;
    ...
    DRJIT_STRUCT(
        ...
        transition_edge_v0_x,
        transition_edge_v0_y,
        transition_edge_v0_z,
        transition_edge_v1_x,
        transition_edge_v1_y,
        transition_edge_v1_z,
        adjacent_valid,
        adjacent_plane_point_x,
        adjacent_plane_point_y,
        adjacent_plane_point_z,
        adjacent_plane_normal_x,
        adjacent_plane_normal_y,
        adjacent_plane_normal_z,
        adjacent_eta_r,
        adjacent_mu_r,
        adjacent_sigma,
        adjacent_gain,
...
struct FWeightReflectionOpInput {
    Int32 path_idx;
    Int32 rx_idx;
    Int32 valid_mask;
    FWeightReflectionOpArrays arrays;
    float tx_pos_x;
    float tx_pos_y;
    float tx_pos_z;
    float tx_pol_x;
...
inline void launch_reflection_forward(
    ...
    witwin::channel::native_ext::reflection_accumulate_forward(
        ...
        drjit_data_ptr(input.arrays.slot_mu_r),
...
inline void launch_reflection_f_weight_forward(
    const drjit::detached_t<FWeightReflectionOpInput> &input,
    ReflectionOpOutput &output
) {
    drjit::eval(
        input.path_idx,
    ...
    drjit::eval(
        ...
        input.arrays.slot_sigma,
        input.arrays.slot_gain,
        input.arrays.transition_support_valid,
        input.arrays.transition_primary_side,
        input.arrays.transition_edge_distance,
        input.arrays.adjacent_valid,
        input.arrays.adjacent_plane_point_x,
        input.arrays.adjacent_plane_point_y,
        input.arrays.adjacent_plane_point_z,
        input.arrays.adjacent_plane_normal_x,
        input.arrays.adjacent_plane_normal_y,
        input.arrays.adjacent_plane_normal_z
    );
    drjit::eval(
        input.arrays.adjacent_eta_r,
        input.arrays.adjacent_mu_r,
        input.arrays.adjacent_sigma,
        input.arrays.adjacent_gain,
        input.arrays.rx_x,
        input.arrays.rx_y,
        input.arrays.rx_z,
        output.vec_x_re,
    ...
    witwin::channel::native_ext::reflection_accumulate_f_weight_forward(
        drjit_data_ptr(input.path_idx),
        drjit_data_ptr(input.rx_idx),
        drjit_data_ptr(input.valid_mask),
        drjit_data_ptr(input.arrays.image_source_x),
        ...
        drjit_data_ptr(input.arrays.slot_sigma),
        drjit_data_ptr(input.arrays.slot_gain),
        drjit_data_ptr(input.arrays.transition_support_valid),
        drjit_data_ptr(input.arrays.transition_primary_side),
        drjit_data_ptr(input.arrays.transition_edge_distance),
        drjit_data_ptr(input.arrays.adjacent_valid),
        drjit_data_ptr(input.arrays.adjacent_plane_point_x),
        drjit_data_ptr(input.arrays.adjacent_plane_point_y),
        drjit_data_ptr(input.arrays.adjacent_plane_point_z),
        drjit_data_ptr(input.arrays.adjacent_plane_normal_x),
        drjit_data_ptr(input.arrays.adjacent_plane_normal_y),
        drjit_data_ptr(input.arrays.adjacent_plane_normal_z),
        drjit_data_ptr(input.arrays.adjacent_eta_r),
        drjit_data_ptr(input.arrays.adjacent_mu_r),
        drjit_data_ptr(input.arrays.adjacent_sigma),
        drjit_data_ptr(input.arrays.adjacent_gain),
        drjit_data_ptr(input.arrays.rx_x),
        drjit_data_ptr(input.arrays.rx_y),
        drjit_data_ptr(input.arrays.rx_z),
        input.tx_pos_x,
...
inline void launch_reflection_f_weight_jvp(
    const drjit::detached_t<FWeightReflectionOpInput> &input,
    const Float &t_image_source_x,
    const Float &t_image_source_y,
    const Float &t_image_source_z,
    ...
    const Float &t_transition_edge_v0_x,
    const Float &t_transition_edge_v0_y,
    const Float &t_transition_edge_v0_z,
    const Float &t_transition_edge_v1_x,
    const Float &t_transition_edge_v1_y,
    const Float &t_transition_edge_v1_z,
    const Float &t_adjacent_plane_point_x,
    const Float &t_adjacent_plane_point_y,
    const Float &t_adjacent_plane_point_z,
    const Float &t_adjacent_plane_normal_x,
    const Float &t_adjacent_plane_normal_y,
    const Float &t_adjacent_plane_normal_z,
    const Float &t_rx_x,
    const Float &t_rx_y,
    const Float &t_rx_z,
    drjit::detached_t<ReflectionOpOutput> &output
    ...
    drjit::eval(
        ...
        input.arrays.transition_primary_side,
        input.arrays.transition_edge_distance,
        input.arrays.transition_edge_v0_x,
        input.arrays.transition_edge_v0_y,
        input.arrays.transition_edge_v0_z,
        input.arrays.transition_edge_v1_x,
        input.arrays.transition_edge_v1_y,
        input.arrays.transition_edge_v1_z,
        input.arrays.adjacent_valid,
        input.arrays.adjacent_plane_point_x,
        input.arrays.adjacent_plane_point_y,
        input.arrays.adjacent_plane_point_z,
    ...
    drjit::eval(
        ...
        t_transition_edge_v0_x,
        t_transition_edge_v0_y,
        t_transition_edge_v0_z,
        t_transition_edge_v1_x,
        t_transition_edge_v1_y,
        t_transition_edge_v1_z,
        t_adjacent_plane_point_x,
        t_adjacent_plane_point_y,
        t_adjacent_plane_point_z,
        t_adjacent_plane_normal_x,
        t_adjacent_plane_normal_y,
        t_adjacent_plane_normal_z,
        t_rx_x,
        t_rx_y,
        t_rx_z,
        output.vec_x_re,
    ...
    witwin::channel::native_ext::reflection_accumulate_f_weight_jvp(
        drjit_data_ptr(input.path_idx),
        drjit_data_ptr(input.rx_idx),
        drjit_data_ptr(input.valid_mask),
        drjit_data_ptr(input.arrays.image_source_x),
        ...
        drjit_data_ptr(input.arrays.transition_primary_side),
        drjit_data_ptr(input.arrays.transition_edge_distance),
        drjit_data_ptr(input.arrays.transition_edge_v0_x),
        drjit_data_ptr(input.arrays.transition_edge_v0_y),
        drjit_data_ptr(input.arrays.transition_edge_v0_z),
        drjit_data_ptr(input.arrays.transition_edge_v1_x),
        drjit_data_ptr(input.arrays.transition_edge_v1_y),
        drjit_data_ptr(input.arrays.transition_edge_v1_z),
        drjit_data_ptr(input.arrays.adjacent_valid),
        drjit_data_ptr(input.arrays.adjacent_plane_point_x),
        drjit_data_ptr(input.arrays.adjacent_plane_point_y),
        drjit_data_ptr(input.arrays.adjacent_plane_point_z),
        ...
        drjit_data_ptr(t_slot_plane_normal_y),
        drjit_data_ptr(t_slot_plane_normal_z),
        drjit_data_ptr(t_transition_edge_distance),
        drjit_data_ptr(t_transition_edge_v0_x),
        drjit_data_ptr(t_transition_edge_v0_y),
        drjit_data_ptr(t_transition_edge_v0_z),
        drjit_data_ptr(t_transition_edge_v1_x),
        drjit_data_ptr(t_transition_edge_v1_y),
        drjit_data_ptr(t_transition_edge_v1_z),
        drjit_data_ptr(t_adjacent_plane_point_x),
        drjit_data_ptr(t_adjacent_plane_point_y),
        drjit_data_ptr(t_adjacent_plane_point_z),
        drjit_data_ptr(t_adjacent_plane_normal_x),
        drjit_data_ptr(t_adjacent_plane_normal_y),
        drjit_data_ptr(t_adjacent_plane_normal_z),
        drjit_data_ptr(t_rx_x),
        drjit_data_ptr(t_rx_y),
        drjit_data_ptr(t_rx_z),
        drjit_data_ptr_mut(output.vec_x_re),
...
class FWeightReflectionAccumulateOp
    : public WitwinCustomOp<ReflectionOpOutput, FWeightReflectionOpInput> {
public:
    using Base = WitwinCustomOp<ReflectionOpOutput, FWeightReflectionOpInput>;
    using OutputType = typename Base::OutputType;

    explicit FWeightReflectionAccumulateOp(const FWeightReflectionOpInput &input)
        : Base(input) {}

    OutputType eval(drjit::detached_t<FWeightReflectionOpInput> input) {
        m_input = input;
        OutputType output = zero_reflection_output(input.arrays.rx_x.size());
        if (input.n_pairs > 0) {
            launch_reflection_f_weight_forward(input, output);
        }
        return output;
    }
    ...
    void forward() override {
        ...
        if (m_input.n_pairs > 0) {
            ...
            launch_reflection_f_weight_jvp(
                m_input,
                coerce(drjit::grad<false>(this->m_registered_input.arrays.image_source_x), m_input.arrays.image_source_x.size()),
                coerce(drjit::grad<false>(this->m_registered_input.arrays.image_source_y), m_input.arrays.image_source_y.size()),
                coerce(drjit::grad<false>(this->m_registered_input.arrays.image_source_z), m_input.arrays.image_source_z.size()),
                ...
                coerce(drjit::grad<false>(this->m_registered_input.arrays.slot_plane_normal_y), m_input.arrays.slot_plane_normal_y.size()),
                coerce(drjit::grad<false>(this->m_registered_input.arrays.slot_plane_normal_z), m_input.arrays.slot_plane_normal_z.size()),
                coerce(drjit::grad<false>(this->m_registered_input.arrays.transition_edge_distance), m_input.arrays.transition_edge_distance.size()),
                coerce(drjit::grad<false>(this->m_registered_input.arrays.transition_edge_v0_x), m_input.arrays.transition_edge_v0_x.size()),
                coerce(drjit::grad<false>(this->m_registered_input.arrays.transition_edge_v0_y), m_input.arrays.transition_edge_v0_y.size()),
                coerce(drjit::grad<false>(this->m_registered_input.arrays.transition_edge_v0_z), m_input.arrays.transition_edge_v0_z.size()),
                coerce(drjit::grad<false>(this->m_registered_input.arrays.transition_edge_v1_x), m_input.arrays.transition_edge_v1_x.size()),
                coerce(drjit::grad<false>(this->m_registered_input.arrays.transition_edge_v1_y), m_input.arrays.transition_edge_v1_y.size()),
                coerce(drjit::grad<false>(this->m_registered_input.arrays.transition_edge_v1_z), m_input.arrays.transition_edge_v1_z.size()),
                coerce(drjit::grad<false>(this->m_registered_input.arrays.adjacent_plane_point_x), m_input.arrays.adjacent_plane_point_x.size()),
                coerce(drjit::grad<false>(this->m_registered_input.arrays.adjacent_plane_point_y), m_input.arrays.adjacent_plane_point_y.size()),
                coerce(drjit::grad<false>(this->m_registered_input.arrays.adjacent_plane_point_z), m_input.arrays.adjacent_plane_point_z.size()),
                coerce(drjit::grad<false>(this->m_registered_input.arrays.adjacent_plane_normal_x), m_input.arrays.adjacent_plane_normal_x.size()),
                coerce(drjit::grad<false>(this->m_registered_input.arrays.adjacent_plane_normal_y), m_input.arrays.adjacent_plane_normal_y.size()),
                coerce(drjit::grad<false>(this->m_registered_input.arrays.adjacent_plane_normal_z), m_input.arrays.adjacent_plane_normal_z.size()),
                coerce(drjit::grad<false>(this->m_registered_input.arrays.rx_x), m_input.arrays.rx_x.size()),
                coerce(drjit::grad<false>(this->m_registered_input.arrays.rx_y), m_input.arrays.rx_y.size()),
                coerce(drjit::grad<false>(this->m_registered_input.arrays.rx_z), m_input.arrays.rx_z.size()),
                output
    ...
    const char *name() const override { return "FWeightReflectionAccumulate"; }

private:
    drjit::detached_t<FWeightReflectionOpInput> m_input;
};
...
void register_reflection_bindings(nb::module_ &m) {
    ...
    m.def(
        "reflection_prefix_compact_representatives",
        [](
            Int32 bounce_count,
            Int32 discovery_count,
            Int32 representative_ray_index,
            Int32 global_prim_ids,
            Float image_source_x,
            Float image_source_y,
            Float image_source_z,
            Int32 canonical_prim_table,
            int ray_count,
            int max_bounces,
            int depth,
            ...
            Int32 out_representative_chain_idx = drjit::zeros<Int32>(static_cast<size_t>(ray_count));
            Int32 out_discovery_count = drjit::zeros<Int32>(static_cast<size_t>(ray_count));
            ...
            witwin::channel::native_ext::reflection_prefix_compact_representatives(
                drjit_data_ptr(bounce_count),
                drjit_data_ptr(discovery_count),
                drjit_data_ptr(representative_ray_index),
                drjit_data_ptr(global_prim_ids),
                drjit_data_ptr(image_source_x),
                drjit_data_ptr(image_source_y),
                drjit_data_ptr(image_source_z),
                canonical_prim_table.size() > 0 ? drjit_data_ptr(canonical_prim_table) : nullptr,
                static_cast<int>(canonical_prim_table.size()),
                ray_count,
                max_bounces,
                depth,
                image_source_tolerance,
                drjit_data_ptr_mut(out_count),
                drjit_data_ptr_mut(out_representative_chain_idx),
                drjit_data_ptr_mut(out_discovery_count)
            );
        ...
        "Exact native prefix compaction for RayD reflection chain representatives."
    );
    ...
    m.def(
        "reflection_accumulate_f_weight",
        [](
            Int32 path_idx,
            Int32 rx_idx,
            Int32 valid_mask,
            ...
            FWeightReflectionOpInput input{
                path_idx,
                rx_idx,
                valid_mask,
                {
            ...
            ReflectionOpOutput output = witwin_custom_op<FWeightReflectionAccumulateOp>(input);
            return nb::make_tuple(
                output.vec_x_re,
                output.vec_x_im,
                output.vec_y_re,
        ...
        "Reflection F-weight accumulation with CUDA-side transition and adjacent residual evaluation."
    );
}
